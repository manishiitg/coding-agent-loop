...
The memory system provides three virtual tools — `save_memory`, `recall_memory`, and `compress_memory` — available in **Multi-Agent Chat mode** (delegation mode: `plan`). Each tool spawns an asynchronous background sub-agent that performs operations using shell commands. The manager agent is notified when the background agent completes.
...
1. Creates the current day folder (e.g., `Plans/memories/YYYY-MM-DD/`)
...
| Parameter | Required | Description |
|-----------|----------|-------------|
| `focus`     | No       | Optional topic to focus the compression on. |

Returns immediately with an `agent_id`. A background agent (running with `medium` reasoning):
1. **Analyzes:** Reads all date and entity files. Identifies redundant, superseded, duplicate, or overly verbose entries.
2. **Rewrites:** Consolidates and deduplicates entries, rewriting the markdown files cleanly and removing empty folders/stale entities.
3. **Regenerates Index:** Builds a fresh `index.md` file containing key decisions, project state, active entities, and orientation guidance for future sessions.

## Storage Structure

The system uses parallel structures for different access patterns, defaulting to `Plans/memories/` (this path can be overridden per session via the `memory_folder` context key):

```
Plans/memories/
  index.md               ← High-level snapshot (regenerated by compress_memory)
  ...
  2026-03-08/            ← Daily date folders (chronological log)
    general.md
    decisions.md
    preferences.md
```
...
Generated by `compress_memory`, this is the primary orientation file read at the start of a session or before making architectural decisions. It contains:
...
Entity files group all knowledge about a specific named thing (project, system, technology, person, feature). They provide O(1) lookup speed. They are updated by `save_memory` when it extracts relevant entities, and cleaned up by `compress_memory`.
...
```
Manager Agent
  ↓ calls save_memory() / recall_memory() / compress_memory()
  ↓
...
|-------------|-------|---------|
| `MemoryFolderKey` | `memory_folder` string | Overrides the default `Plans/memories` path |
...
| `agent_go/cmd/server/server.go` | Tool registration and context injection in plan mode |
