...
var referenceKinds = map[string]kindMeta{
	...
	"integration-discovery": {Group: "system", Description: "How to connect a third-party service (\"I want to use ClickUp/Notion/Stripe in this workflow\"): check three avenues in order — CLI first (token-efficient, preferred), then a skill wrapping it, then MCP last — using search_mcp_catalog, search_skills, and execute_shell_command/web search for CLI discovery. Covers the preference order, why, and how to report findings and help the user connect whichever fits. Load whenever a user asks to integrate or connect a new service/tool.", Modes: []string{"multi-agent", "workshop"}, Tools: []string{"search_mcp_catalog", "search_skills", "install_skill", "install_mcp_server", "add_mcp_server"}},
