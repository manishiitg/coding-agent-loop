...
var builderInvariantCertifications = map[BuilderInvariantID]BuilderInvariantCertification{
	...
	InvCostTrackingPipeline: {
		...
		Description: "Real Anthropic API call → adapter GenerationInfo → orchestrator token event → cost ledger persist → /api/cost HTTP surface. Per-CLI variants exist (cost_http_e2e_codex/cursor/pi_real_test.go) that prove the same pipeline for each CLI provider; cost_http_e2e_real_test.go covers the API-provider HTTP path.",
		RealE2E:     true,
	},
	InvInspectorDebugVisibility: {
		...
		Env:         []string{"RUN_ANTHROPIC_REAL_E2E=1", "ANTHROPIC_API_KEY"},
		Description: "Drives a real Anthropic turn through the orchestrator and asserts /api/inspector returns the full event chain (system prompt, LLM generation, streaming chunks, token usage) for the live session. Per-CLI variants (inspector_e2e_cursor/pi_real_test.go) prove the same surface for CLI providers. Companion TestInspectorHTTPUnknownSessionReturns404 covers the empty-session branch.",
		RealE2E:     true,
	},
	InvProviderAPIKeyValidation: {
