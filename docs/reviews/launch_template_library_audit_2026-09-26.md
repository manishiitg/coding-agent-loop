...
| Multi-Crew proposal and typed handoffs | 27 locally installable multi-Crew Playbooks inside 47 total packages; 32 contract suites pass `python3 playbooks/scripts/validate_playbooks.py` | Authored and mechanically checked. Builder must still insert blocking validation in an actual Workflow. |
...
The other **20 of 47** installable Playbook packages have no `agent_slots`, `SETUP.json`, or executable handoff suite. They remain Workflow guides in the same catalog; they are not yet equivalent to the 27 chat-led multi-Crew proposals. The Playbook picker now labels the two kinds separately and states that Workflow guides have no predefined Crew team or tracked setup checklist; an installer test confirms this for Basic Browser Setup. This count excludes the example package under `playbooks/templates/`.
...
| Growth Analytics (4) | Growth Data Foundation; Funnel and Conversion Intelligence; Activation and Retention Intelligence; Growth Experimentation and Follow-Through. |
