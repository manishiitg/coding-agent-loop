...
7. **In-widget transforms (`sql`)**: push joins, aggregation, sorting, and limits into the widget's `sql` query rather than maintaining pre-flattened helper tables or a workflow `step-generate-report` flatten step. A `SELECT ... JOIN ... GROUP BY ... ORDER BY ... LIMIT` against the canonical tables returns exactly the rows the widget needs. Use this to **collapse derived/helper tables** back to their canonical sources.
