          ...
          {panel.filteredJobs.map(job => {
            ...
            return <React.Fragment key={job.id}>
              <tr className="transition-colors hover:bg-muted/20">
                ...
                <td className="max-w-56 px-3 py-3 text-xs text-muted-foreground"><span className="line-clamp-2" title={describeCron(job.cron_expression)}>{describeCron(job.cron_expression)}</span></td>
                ...
                <td className="whitespace-nowrap px-3 py-3 text-xs">{!job.enabled ? '—' : panel.isSchedulerPaused ? <span className="text-muted-foreground">Paused globally</span> : formatLocalScheduleTime(job.next_run_at)}</td>
              ...
              {open && <tr><td colSpan={6} className="bg-muted/15 px-4 py-4">
                <div id={detailsId} role="region" aria-label={`${name} details`} className="min-h-36 space-y-3">
                  ...
                  <p className="text-xs text-muted-foreground">Ask this automation in Chat to change its schedule.</p>
                </div>
              </td></tr>}
            </React.Fragment>
          })}
