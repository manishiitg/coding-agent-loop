...
- **Document escape hatch:** for a rich/narrative/highly-custom report the widget grammar can't express cleanly (or when the user dislikes the widget-composed look), **author** a `.md`/`.html` document **once** under `db/reports/` and render it with one `file` widget (`renderFormat: "markdown"` or `"html"`). **Do NOT add a workflow step to (re)generate the report each run** — wire the document to read data LIVE so it stays current with zero per-run work: HTML reads `db/db.sqlite` live via the `window.report` API; markdown embeds live db-bound widgets via ` ```report-widget ` blocks. The workflow's normal steps already write the data to `db/`; the report just reads it. (Only a doc with data **hardcoded at author time** is static and would need regeneration — the discouraged path; bake data in only for a deliberate one-off export.) HTML renders in a sandboxed iframe (inline CSS/JS, tables, charts, dark mode all work); links to PDFs/files inside it are clickable. See `get_reference_doc(kind="reporting-policy")` for the full dispatch.
