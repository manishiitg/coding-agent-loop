...
- **db/db.sqlite** — **workflow state and results**. A SQLite database with one table per entity that this step produces/consumes (processed records, cursors, cumulative output). Owned by your step. Read with `+"`sqlite3 db/db.sqlite \"SELECT ...\"`"+` and write with `+"`INSERT ... ON CONFLICT(<pk>) DO UPDATE SET ...`"+` (upsert on the table's primary key). NEVER DROP/recreate a table or delete-then-insert the whole table — that destroys rows from other groups/runs. Schema/contract per table is in `+"`db/README.md`"+`. (Single-quote SQL in the shell; JSON paths like `+"`'$.field'`"+` need quoting.)
...
- **builder/** — prior review/improvement context. At step start, read `+"`builder/review.html`"+` and `+"`builder/improve.html`"+` if they exist. Use unresolved findings, prior failed approaches, active/deferred improvement ideas, and resolved markers as context so you do not repeat known mistakes. Treat these logs as READ-ONLY during step execution.
