...
export function VertexSection({ config, onUpdate, onTestAPIKey, apiKeyStatus, apiKeyError, metadata }: VertexSectionProps) {
  ...
  const { availableVertexModels, customVertexModels, addCustomVertexModel, removeCustomVertexModel, refreshAvailableLLMs, saveLLM, testAPIKey: testAPIKeyFromStore, lockedProviders, llmConfigLocked } = useLLMStore()
  
  const isLocked = llmConfigLocked || lockedProviders.includes('vertex')
      ...
      <Card className="p-4">
        ...
        <div className="space-y-4">
          ...
          <div className="border-t border-border pt-4 space-y-3">
            ...
            {config.model_id && config.model_id.startsWith('claude-') && !isLocked && (
            ...
            <div className="space-y-2">
              <div className="flex gap-2">
                <input type="password" value={apiKey} onChange={(e) => handleAPIKeyChange(e.target.value)} placeholder={isLocked ? "••••••••••••••••" : "Enter your Vertex AI API key (optional)"} className="flex-1 px-3 py-2 border border-border rounded-md bg-background text-foreground focus:ring-2 focus:ring-primary focus:border-primary" disabled={isLocked} />
                {!isLocked && (
              ...
              {!apiKey && !isLocked && <div className="text-xs text-muted-foreground">No API key provided - will test using OAuth authentication (gcloud/service account/ADC).</div>}
              ...
              {!isLocked && apiKeyStatus === 'valid' && <div className="text-sm text-green-600 dark:text-green-400 flex items-center gap-1"><CheckCircle className="w-4 h-4" />Authentication successful</div>}
              {!isLocked && apiKeyStatus === 'invalid' && <div className="text-sm text-red-600 dark:text-red-400 flex items-center gap-1"><AlertCircle className="w-4 h-4" />{apiKeyError || 'Authentication failed'}</div>}
            </div>
          </div>
