...
var builtinOrgPulseMessages = []string{
	...
	`STEP 4/6 — GENERATE RECOMMENDATIONS. This is the proactive step: for each goal, propose grounded, prioritized recommendations to MOVE it — not just diagnose it. Follow get_reference_doc(kind="org-pulse") "Generate recommendations". First read existing org-level recommendation cards in pulse/goals.html and workflow-level Chief of Staff cards in each builder/improve.html (` + "`.cos-rec`" + ` / data-cos-rec-id / data-status). For any existing open rec, report whether it is proposed, accepted, queued_goal_advisor, in_progress, needs_evidence, blocked, done, or dismissed; update/follow up instead of duplicating. Every new rec is tied to a goal + evidence and ranked by impact/effort, and is PROPOSAL-ONLY — you recommend, the user/builder decides; never auto-apply a plan change. Think beyond the obvious: a new automation for an unserved goal, a different approach for a capped goal, cross-automation synergies, and promotions. Write per-automation improvement recs into that automation's builder/improve.html as structured Chief of Staff recommendation cards with stable data-cos-rec-id, data-status="proposed", data-priority, data-suggested-action, evidence, gap, and expected impact. Write org-level recs to the Recommendations section of pulse/goals.html per get_reference_doc(kind="org-html"). Report recommendations written, existing recommendations followed up, stale open decisions, and where each lives, then stop.`,
