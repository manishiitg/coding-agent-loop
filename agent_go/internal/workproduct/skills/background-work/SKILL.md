...
description: Run a bounded research, analysis, writing, coding, or file task asynchronously and rely on Crew's automatic completion notification instead of polling.
---
...
After the tool returns, tell the user the task is running and end the turn. Do not poll with `query_agent` or sleep loops. Crew automatically sends an `[AUTO-NOTIFICATION]` into this same chat when the child completes or fails; then summarize the result and continue only if more work is required.
