...
var referenceKinds = map[string]kindMeta{
	...
	"post-run-monitor":    {Group: "system", Description: "The post-run monitor playbook: read run evidence + changelog + eval files, form a Bug verdict (ran-right) and a Goal verdict (hits-success-criteria, gated on Bug being clean), call harden_workflow for real Bugs, then run separate report-only Artifact Review and LLM/model/cost/time steps, back up the final state, publish, and notify on transitions. Update builder/improve.html (verdict pills, goal card, signal tiles, Artifact Review/cost/time notes, a run row, and a Monitor/Note entry only if something's wrong or materially notable); the verdict pills + headline in improve.html are the notification-gate signal. Triage is diagnosis/verdict only; fix/harden starts the canonical harden_workflow repair tool; Artifact Review and LLM/cost/time are report-only. Load when running the post-run monitor pass.", Modes: []string{"workshop", "run"}},
