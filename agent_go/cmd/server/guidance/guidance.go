...
var allKinds = map[string]kindMeta{
	...
	"improve-workflow":    {Group: "improve", Description: "Unified evidence-driven workflow improvement across plan, eval, reports, KB, learnings, and db: harden, replan, or clean up evidence stores from run/eval evidence", Modes: []string{"workshop"}},
	...
	"improve-report":      {Group: "report", Description: "Report accuracy, live-data wiring, layout, color, density, and responsive design improvements", Modes: []string{"workshop"}},
}
...
var referenceKinds = map[string]kindMeta{
	...
	"review-improve-log":  {Group: "system", Description: "The single workflow log spec: builder/improve.html format (newest-on-top timeline of run/monitor/decision/user-rule/note/open-finding entries, Bug/Goal plus cost/time prose tiles, per-run LLM/model/cost/time readout, monthly archive), a copy-paste starter HTML skeleton, the prose close-out ritual, and the one-time migration off legacy .md / review.html / F-/I- ids / improve-decision blocks. Load before writing the log from any /review-* or /improve-* skill or the monitor.", Modes: []string{"workshop", "run"}},
	"post-run-monitor":    {Group: "system", Description: "The post-run monitor playbook: read run evidence + changelog + eval files, form a Bug verdict (ran-right) and a Goal verdict (hits-success-criteria, gated on Bug being clean), call harden_workflow for real Bugs, then run a separate report-only LLM/model/cost/time step by plan step and agent when evidence allows, back up the final state, publish, and notify on transitions. Update builder/improve.html (verdict pills, goal card, signal tiles, cost/time tiles, a run row, and a Monitor/Note entry only if something's wrong or materially notable); the verdict pills + headline in improve.html are the notification-gate signal. Triage is diagnosis/verdict only; fix/harden starts the canonical harden_workflow repair tool; the LLM/cost/time audit is report-only. Load when running the post-run monitor pass.", Modes: []string{"workshop", "run"}},
	...
	"report-plan":         {Group: "system", Description: "Report toolchain: a report is HTML document(s) registered in report_plan.json (no widget grammar, no other format). HTML reads db/db.sqlite live via the window.report API and renders its own visuals. Covers the window.report API, the good-document + design-quality + theming + responsive guide, tabs, per-report themes, validate/preview. Load before authoring or editing reports/report_plan.json.", Modes: []string{"workshop"}},
