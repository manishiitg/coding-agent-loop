...
- **MCP**: `search_mcp_catalog(query)`. This checks three sources in one call:
  ...
  - **GitHub's public MCP Registry** and **Smithery's directory** — much larger, but **unvetted**. A hit here is a lead, not a recommendation: before adding it with `add_mcp_server`, tell the user what you found (name, description, whether it's remote/HTTP or needs a local package run) and let them decide, the same way a human would evaluate any third-party integration before installing it. Do not silently `add_mcp_server` an unvetted hit.
