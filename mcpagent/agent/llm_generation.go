...
func GenerateContentWithRetry(a *Agent, ctx context.Context, messages []llmtypes.MessageContent, opts []llmtypes.CallOption, turn int, sendMessage func(string)) (*llmtypes.ContentResponse, observability.UsageMetrics, error) {
	...
	for attempt := 0; attempt < maxRetries; attempt++ {
		...
		logger.Infof("🔄 [DEBUG] GenerateContentWithRetry attempt %d - About to call a.LLM.GenerateContent", attempt+1)

		llmCallStart := time.Now()
		resp, err := a.LLM.GenerateContent(ctx, messages, opts...)
		llmCallDuration := time.Since(llmCallStart)
		logger.Infof("🔄 [DEBUG] GenerateContentWithRetry attempt %d - Duration: %v, Error: %v", attempt+1, llmCallDuration, err != nil)

		if err == nil {
			logger.Infof("🔄 [DEBUG] GenerateContentWithRetry attempt %d - SUCCESS", attempt+1)
			usage = extractUsageMetricsWithMessages(resp, messages)
			return resp, usage, nil
		}

		// Error handling
		logger.Infof("🔄 [DEBUG] GenerateContentWithRetry attempt %d - ERROR: %v", attempt+1, err)
		lastErr = err
