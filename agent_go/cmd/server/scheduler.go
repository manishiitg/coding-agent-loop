...
func postRunMonitorGateStep(pulseRunID, runFolder, runStatus string) postRunMonitorStep {
	return postRunMonitorStep{
		...
		query: fmt.Sprintf("STEP 1 — PULSE GATE / WORKLIST. This is the only mandatory intelligence step for this Pulse run. pulse_run_id=%q, run_folder=%q, run_status=%q.\n\n"+
			"Use a progressive evidence scan, not a full audit. First read builder/improve.html (authoritative history/current cadence), including any .advisor-experiment card and its data-status/data-review-after, get_pulse_module_state(workspace_path=\"<current workflow>\"), latest run metadata/summary, changelog filenames and review flags, existence/freshness metadata for plan/eval/report artifacts, the workflow.json version plus a compact resolved LLM/tier/fallback signature, backup/publish/notification readiness metadata, db/README or a compact schema summary, per-step learning metadata, a compact KB note index, open/answered report_human_inputs, and pending Chief of Staff recommendation cards. Do not load full report HTML, full KB/learnings, broad DB rows, long logs, or every cost file unless a compact signal makes that module plausibly due or you need one targeted fact to justify its decision. The selected module performs the deep audit later; Gate only chooses the evidence-backed worklist and cadence. If the scan finds a plausible bug signal, mark Bug Review due so its read-only reviewer can investigate and the Pulse Fixer can repair and verify it. Do not treat the SQLite module cache as historical truth.\n\n"+
			...
			"Module meanings:\n"+
			"- bug_review: due for real Bug signals, failed producing steps, stale guards, selector/API/runtime breakage, workflow code that reuses an older receipt/artifact, broken evidence contracts, hallucinated success, or CoS recommendations that are operational bugs. The reviewer is read-only; Pulse Fixer writes.\n"+
			...
			"- llm_ops_review: low-frequency workflow coaching for execution tier usage, resolved provider/model/fallback routing, Pulse and Maintenance models, and publish/notify/backup/version readiness. It proposes exact evidence-backed changes through the existing Needs your decision cards and never silently rewrites configuration.\n"+
...
func postRunMonitorModuleSteps(pulseRunID string) []postRunMonitorModuleStep {
	steps := []postRunMonitorModuleStep{
		{pulseModuleBugReview, postRunMonitorStep{"bug-review", fmt.Sprintf("PULSE MODULE — BUG REVIEW. pulse_run_id=%q. This is a read-only reliability review selected by Pulse Gate. Inspect the compact Gate worklist, retained run/eval evidence, execution logs, validation, prompts/config, stale artifacts, selector/API/runtime failures, hallucinated success, and report/eval evidence-chain breakage. Return verdict, ordered findings, exact evidence, bounded recommended fixes, verification steps, and whether user judgment is required. Do not edit files, call mutation tools, update builder/improve.html, ask the user, or mark module state from the reviewer. The parent Pulse Fixer consolidates this review with all other due modules, applies safe fixes sequentially, records one `Bug fix` outcome when needed, and calls mark_pulse_module_result(workspace_path=\"<current workflow>\", pulse_run_id=%q, module=\"bug_review\", result=\"done|changed|blocked|failed|skipped\", reason=\"...\", evidence=[...]).", pulseRunID, pulseRunID)}},
