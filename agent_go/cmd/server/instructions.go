...
Returns the canonical guided-flow text for any workflow slash command. Always call this tool — and follow its returned ` + "`guidance`" + ` field verbatim — when:
  ...
  1. The user invokes a slash command (` + "`/design-plan`" + `, ` + "`/run-goal-work`" + `, etc.). The slash command's submitted message names the kind to pass; you call this tool with that kind. Do NOT improvise the flow yourself.
...
**Kinds — match to intent:**
  ...
  Improvements:
    ...
    - pulse-fixer              → the Fix phase of a Technical Review+Fix sequence: apply safe fixes to the findings that review filed
...
Pulse is the single broad maintenance path. Its main job is Goal Work: doing work that moves the user's goals. Plan Drift, Technical and Architecture reviews keep the workflow healthy, and every issue they find ends the same pass fixed, closed as not a problem, sent to the user as a decision, or handed to the platform. The full Pulse runs on the workflow's own self-paced schedule; fix runs (Technical Review+Fix) start automatically when a workflow has open issues, new step concerns or a failed run. In chat, ` + "`/pulse`" + ` runs the full Pulse once and ` + "`/run-goal-work`" + ` runs a Goal Work pass with an optional focus (` + "`/strategy-auditor`" + ` and ` + "`/goal-advisor`" + ` are aliases). Plan Drift, Technical and Architecture are started from the Pulse tab, not from chat commands.
...
func buildWorkflowContextPrompt(paths []string, _ string) string {
	return buildWorkflowContextPromptWithLabels(paths, nil)
}

// workflowContextRef is one # reference as the user saw it in the composer.
type workflowContextRef struct {
	Path  string `json:"path"`
	Label string `json:"label"`
	Kind  string `json:"kind,omitempty"`
}

// workflowContextLabels maps each reference's canonical path to a sanitized
// display label. Labels are client-supplied display text only.
func workflowContextLabels(refs []workflowContextRef) map[string]string {
	labels := make(map[string]string, len(refs))
	for _, ref := range refs {
		label := strings.Join(strings.Fields(strings.NewReplacer("`", "", "*", "").Replace(ref.Label)), " ")
		if len([]rune(label)) > 80 {
			label = string([]rune(label)[:80])
		}
		key := normalizeConversationWorkspace(strings.TrimSuffix(strings.TrimSpace(ref.Path), "/"))
		if label != "" && key != "" {
			labels[key] = label
		}
	}
...
// buildWorkflowContextPromptWithLabels lists each reference under the typed
// tag the user saw (#crew:<label> / #workflow:<label>), with its kind derived
// from the path, so the agent never mistakes a reference for a Slack channel.
func buildWorkflowContextPromptWithLabels(paths []string, labels map[string]string) string {
	if len(paths) == 0 {
		return ""
	}
	...
	for _, wsPath := range paths {
		...
		kind, tagKind := "workflow", "workflow"
		if isCrewProjectPath(wsPath) {
			kind, tagKind = "Crew", "crew"
		}
		label := labels[normalizeConversationWorkspace(wsPath)]
		if label == "" {
			label = path.Base(wsPath)
		}
		references = append(references, fmt.Sprintf("- **#%s:%s** (%s) `%s/`", tagKind, label, kind, wsPath))
	}
	if len(references) == 0 {
		return ""
	}
	return "\n## Workflow Context\n\n" +
		"The following exact AgentWorks workflow and Crew folders are reference context for this message. Workflow folders are read-only: read only the files needed and do not modify or execute them. Crew folders you own (as creator or co-owner) are read-write: you may read and, when the user asks, edit their files. Another owner's Crew folder is read-only: read it, but ask that Crew (call_function) for any change instead of editing its files. The coding CLI starts inside another working directory, so resolve each listed path from the workspace root—for shell calls use `$WORKSPACE_DOCS_PATH/<listed-path>/...`, not `<listed-path>/...` relative to the current directory. Do not list or probe the parent `Workflow/` directory or the parent of an attached Crew: that parent is intentionally not granted, and failure to list that parent does not mean the attached child folder is inaccessible. Verify access against an exact listed folder or file before reporting it unavailable. Start with `workflow.json`, `product.json`, `MEMORY.md`, `soul/soul.md`, `planning/plan.json`, `planning/step_config.json`, `code/`, `db/`, or `builder/conversation/` when relevant, and inspect other files on demand. Treat the files as the source of truth rather than relying on a copied prompt snapshot. A `#crew:<name>` or `#workflow:<name>` tag (or an older bare `#Name`) in the user's message refers to the matching entry below; it is never a Slack channel. Slack channels are always exact IDs like `C1234567890`, never `#name`, so do not offer Slack setup for such a tag. When the user asks to connect to, call, or send work to a tagged Crew or workflow, call one of its functions (`list_functions`, then `call_function` or the generated `<name>__<function>` tool); every Crew and workflow has `ask` for free-form questions and tasks (a workflow's `ask` goes to its assistant, which starts runs with the right variables itself), and a workflow's typed functions check required inputs before anything runs, if available. This list is only what is tagged or attached for this message, not everything you can reach: a Crew or workflow missing from it is not a lost permission, so call it by name with `call_function` (or find it with `list_accessible_workflows`) before calling it unavailable.\n\n" +
