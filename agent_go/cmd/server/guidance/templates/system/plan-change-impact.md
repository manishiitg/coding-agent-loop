...
This proactive check is one end of a loop; the **`review-artifact-drift` audit** (the `review_artifact_sync` tool, which advances the **Artifact Sync Cursor** in `builder/improve.html`) is the agentic backstop that sweeps the changelog and catches anything the proactive pass missed. Do the reconciliation when you make the change so the audit finds nothing to do — then the reconciled changelog tail is yours to trim.
