...
8. **Cost / tier / execution-mode fit** (OPERATIONAL): for each eval step, read its entry in evaluation/step_config.json — specifically `execution_tier` (low / medium / high) and `declared_execution_mode` (code_exec / learn_code). Match the configuration to the eval's actual nature:
