...
5. Run a critical evidence review before choosing an action:
   ...
   - **Evaluation blind spots:** if eval passes despite hallucinated, unsupported, empty, stale, or misreported output, classify the eval layer as weak and update it or log the gap. On every scheduled improve fire, also run the quick eval health check: does each `soul.md` success criterion have an eval step measuring it, does any eval step map to no criterion or duplicate Pulse triage / `pre_validation` (retire it), and is eval spend (`costs/evaluation/`) sane relative to execution spend?
