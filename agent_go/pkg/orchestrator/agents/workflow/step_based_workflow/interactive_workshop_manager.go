...
{{if eq .WorkshopMode "builder"}}- No evaluation design, no `+"`harden_workflow`"+`, no metric cleanup, and no learn_code/script-locking work as part of report authoring. If a report exposes a real workflow-quality problem, fix basic Builder-owned structure/config or switch to Optimizer for hardening/eval/metric work.
...
{{else}}
- No `+"`update_step_config`"+`, no `+"`harden_workflow`"+`, no plan or eval mutations. If the user asks to fix a step's behavior or change what a step writes to `+"`db/`"+`, tell them to switch to Builder or Optimizer.
...
- Do not choose `+"`learn_code`"+`, write `+"`learnings/{step-id}/main.py`"+`, or discuss script locking in Builder mode. Those belong in Optimizer only after explicit user intent plus 10+ scenario-covering successful runs prove a highly deterministic step is safe to freeze.
- If the user explicitly asks for `+"`learn_code`"+`, explain that Builder will first create and test the `+"`code_exec`"+` workflow, then Optimizer can promote only after the user confirms, the step is highly deterministic, and 10+ scenario-covering successful runs prove stability.
...
5. When the notification arrives:
   ...
   - ❌ If failed: report the error clearly. Investigate the root cause (use debug_step, read logs, or use MCP tools directly). {{if or (eq .WorkshopMode "builder") (eq .WorkshopMode "optimizer")}}Fix the step description, config, context wiring, or validation schema, then re-run.{{else}}In this mode, do not mutate workflow artifacts; explain the needed fix and switch to Builder or Optimizer if changes are required.{{end}}
...
{{else if eq .WorkshopMode "optimizer"}}
**Optimizer investigation workflow:**
1. Run **harden_workflow(group_name?)** — reads `+"`iteration-0`"+` eval reports, identifies every failing step, runs the best-practice sweep, and applies targeted fixes automatically.
...
{{else if eq .WorkshopMode "optimizer"}}**The fix should be one of:** update step description (most common), update validation_schema, fix context dependencies, edit/delete learnings, run harden_workflow, or replan from evidence.
{{else if eq .WorkshopMode "reporting"}}**The fix should be one of:** repair report_plan wiring/layout/theme, populate missing db data by running the relevant step, or switch to Builder/Optimizer for workflow/eval/config changes.
{{else}}**The fix should be one of:** explain the issue, rerun/inspect safely, or switch to Builder/Optimizer for any workflow mutation.
{{end}}
...
func registerInteractiveWorkshopTools(iwm *InteractiveWorkshopManager, mcpAgent *mcpagent.Agent, logger loggerv2.Logger) {
	...
	if err := mcpAgent.RegisterCustomTool(
		"list_executions",
		"List all background executions (execute_step, harden_workflow, organize_global_learnings). Shows execution_id, step_id, status (running/done/failed/cancelled), and type. Useful when you need to find execution IDs for query_step or stop_step.",
		map[string]interface{}{
			"type": "object",
			"properties": map[string]interface{}{
	...
	declaredExecutionModeDescription := "Required mode declaration for this step. Always set this intentionally so the optimizer records the final decision explicitly. Builder mode accepts only code_exec. In Optimizer mode, set learn_code only when the user explicitly asked for it, the step is highly deterministic, and 10+ successful runs across relevant scenarios/groups prove the saved script is safe."
	...
	if iwm.currentWorkshopModeFromConfigs(nil) == "builder" {
		...
		declaredExecutionModeDescription = "Builder mode only accepts code_exec. Create and debug the workflow with code_exec steps; learn_code promotion is Optimizer-only and requires explicit user request plus 10+ scenario-covering successful runs."
		lockCodeDescription = "Unavailable in Builder mode. Builder creates and debugs code_exec steps only; lock_code freezes learn_code main.py scripts and is Optimizer-only after explicit user intent plus 10+ scenario-covering successful runs prove the script is stable. Passing lock_code=true in Builder is rejected."
	}
	if err := mcpAgent.RegisterCustomTool(
		"update_step_config",
		...
		map[string]interface{}{
			...
			"properties": map[string]interface{}{
				...
				"review_notes": map[string]interface{}{
					"type":        "string",
					"description": "Free-form rationale covering both why the step is optimized AND why the description is considered reviewed. Cite concrete evidence — e.g., 'description is clear and secret-free; passed 3 groups with eval ≥ 9; learnings stable; pre-validation catches format regressions'. Persisted so later passes (harden, replan, review tools) see the context. Replaces the previous optimized_reason + description_optimization_reason string fields.",
				},
			...
			if workshopMode == "builder" {
				if val, ok := args["declared_execution_mode"]; ok && val != nil {
					if s, ok := val.(string); ok && s == "learn_code" {
						return "Builder mode only creates and debugs code_exec steps. Use declared_execution_mode=\"code_exec\" here. Learn_code promotion is Optimizer-only and requires explicit user request plus 10+ scenario-covering successful runs.", nil
					}
				}
			...
			if val, ok := args["optimized"]; ok && val != nil {
				if b, ok := val.(bool); ok {
					if b {
						...
						// Keep idempotent optimized=true updates cheap when the marker is already set.
						...
						if !alreadyOptimized {
							...
							if stepWritesLearnings {
								if isLearnCodeStep {
									// For scripted code steps: check script exists and has enough scenario-covering evidence.
									...
									if _, readErr := iwm.controller.ReadWorkspaceFile(ctx, mainPyRelPath); readErr != nil {
										...
										if metaContent, readErr := iwm.controller.ReadWorkspaceFile(ctx, metaRelPath); readErr == nil {
											...
											if jsonErr := json.Unmarshal([]byte(metaContent), &meta); jsonErr == nil {
												...
												if scriptedSuccessRuns < 10 {
													missing = append(missing, fmt.Sprintf("scripted code successful runs (%d/10) — keep the step in code_exec/repairable mode until learn_code has 10+ successful scenario-covering runs", scriptedSuccessRuns))
												}
											}
										} else {
											missing = append(missing, "scripted code metadata (script_metadata.json not found — run the step enough times to prove 10+ scenario-covering successful learn_code runs)")
										}
									}
								} else {
									learningFiles, _ := iwm.controller.readStepLearningFiles(ctx, learningsPath)
...
6. Treat an entry as material when it touches description, title, context_dependencies, context_output, validation/pre_validation, step type, route membership, enabled tools/servers/skills, declared_execution_mode, lock_code, lock_learnings, learnings_access, learning_objective, learnings_write_method, knowledgebase_access, knowledgebase_contribution, optimized, or add/delete operations.
...
- lock_code, lock_learnings, or optimized remains after a material change without review_notes proving resync
...
6. **For each passing step** ({{if .GroupName}}passed for `+"`{{.GroupName}}`"+`{{else}}passed ALL groups{{end}}):
   ...
   - If successful_runs >= 3, set optimized=true and lock_learnings=true when the learning notes are stable. For learn_code steps, set lock_code=true only when the user explicitly wanted learn_code, the step is highly deterministic, and script/eval evidence shows 10+ successful runs across the relevant scenario/group surface.
...
func (iwm *InteractiveWorkshopManager) runReviewPlanAgent(ctx context.Context, targetRunFolder string, focus string) (string, error) {
	...
	if stepConfigs, err := iwm.controller.ReadStepConfigs(ctx); err == nil && len(stepConfigs) > 0 {
		...
		for _, sc := range stepConfigs {
			...
			sb.WriteString(fmt.Sprintf("- %s: optimized=%v, mode=%s, declared_mode=%s, successful_runs=%d, lock_learnings=%v, lock_code=%v, learnings_access=%s, learning_objective=%q, learnings_write_method=%s, learning_opted_in=%v, kb_access=%s, kb_contribution=%q, kb_write_method=%s, description_reviewed=%v, review_notes=%q\n", sc.ID, optimized, mode, declaredMode, successfulRuns, lockLearnings, lockCode, learningAccess, learningObjective, learningsWriteMethod, learningOptedIn, kbAccess, kbContribution, kbWriteMethod, descriptionReviewed, reviewNotes))
		}
...
func (iwm *InteractiveWorkshopManager) runReviewArtifactSyncAgent(ctx context.Context, stepID string, focus string) (string, error) {
	...
	if stepConfigs, err := iwm.controller.ReadStepConfigs(ctx); err == nil && len(stepConfigs) > 0 {
		...
		for _, sc := range stepConfigs {
			...
			sb.WriteString(fmt.Sprintf("- %s: optimized=%v, mode=%s, declared_mode=%s, successful_runs=%d, lock_learnings=%v, lock_code=%v, learnings_access=%s, learning_objective=%q, kb_access=%s, kb_contribution=%q, review_notes=%q\n", sc.ID, optimized, mode, declaredMode, successfulRuns, lockLearnings, lockCode, learningAccess, learningObjective, kbAccess, kbContribution, reviewNotes))
		}
		stepConfigSummary = sb.String()
	}
...
func (iwm *InteractiveWorkshopManager) runHardenWorkflowAgent(ctx context.Context, targetRunFolder string, groupName string, focus string) (string, error) {
	...
	if stepConfigs, err := iwm.controller.ReadStepConfigs(ctx); err == nil && len(stepConfigs) > 0 {
		...
		for _, sc := range stepConfigs {
			...
			sb.WriteString(fmt.Sprintf("- %s: optimized=%v, mode=%s, declared_mode=%s, successful_runs=%d, lock_learnings=%v, lock_code=%v, learnings_access=%s, learning_objective=%q, learnings_write_method=%s, kb_access=%s, kb_contribution=%q, kb_write_method=%s, review_notes=%q\n", sc.ID, optimized, mode, declaredMode, successfulRuns, lockLearnings, lockCode, learningsAccess, learningObjective, learningsWriteMethod, kbAccess, kbContribution, kbWriteMethod, reviewNotes))
		}
		stepConfigSummary = sb.String()
	}
