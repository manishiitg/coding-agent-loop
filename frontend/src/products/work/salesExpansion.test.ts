...
describe('expanded Sales templates', () => {
  it('installs three reviewable roles with independent pending setup and examples', () => {
    const templates = crewTemplates.filter(item => ['sales-call-briefing', 'proposal-drafter', 'pipeline-analyst'].includes(item.id))
    expect(templates.map(item => item.id)).toEqual(['sales-call-briefing', 'proposal-drafter', 'pipeline-analyst'])
