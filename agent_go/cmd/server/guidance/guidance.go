...
var allKinds = map[string]kindMeta{
	...
	"improve-workflow":    {Group: "improve", Description: "Manual evidence-driven maintenance across plan, eval, report dashboard quality, KB, learnings, and db: harden, replan, or clean up evidence stores from run/eval evidence", Modes: []string{"workshop"}},
	...
	"goal-advisor":        {Group: "improve", Description: "Expert strategy advisor module: critique goal progress, detect strategic misses, propose out-of-plan ideas, and apply evidence-backed replans when cross-run proof is strong", Modes: []string{"workshop"}},
...
var referenceKinds = map[string]kindMeta{
	...
	"review-improve-log":          {Group: "system", Description: "The single workflow log spec: builder/improve.html format (newest-on-top timeline of run/monitor/maintenance/artifact-review/decision/user-rule/note/open-finding entries, Bug/Goal plus cost/time/Maintenance Radar prose tiles, per-run Artifact Review and LLM/model/cost/time readouts, monthly archive), the prose close-out ritual, and the one-time migration off old Markdown / F-/I- ids / improve-decision blocks. Load before writing the log from any /review-* or /improve-* skill or the monitor.", Modes: []string{"workshop", "run"}},
	"review-improve-log-skeleton": {Group: "system", Description: "Copy-paste starter HTML skeleton for builder/improve.html, including mobile-first Pulse dashboard CSS, Maintenance Radar tiles, filter controls, card examples, decision grids, human-input card shape, and the stable newest-first insertion anchor. Load only when creating a new Pulse log or doing the one-time old-format upgrade required by review-improve-log.", Modes: []string{"workshop", "run"}},
	"post-run-monitor":            {Group: "system", Description: "The dynamic Pulse playbook: Gate reads run evidence, changelog, eval/report/DB/KB/learnings, human inputs, and Chief of Staff recommendations; writes a plain-language Pulse Worklist to builder/improve.html; records selected modules in db/db.sqlite via record_pulse_worklist; then only due modules run (harden, artifact review, report health, eval health, learning health, KB health, DB health, cost/LLM/time, Goal Advisor) before final backup, publish, and a per-run notification. Goal Advisor is selected by Pulse Gate, not a separate recurring schedule. Load when running the post-run monitor pass.", Modes: []string{"workshop", "run"}},
	...
	"workflow-tools":        {Group: "system", Description: "Full reference for workshop / workflow tools: step execution & inspection (execute_step, query_step, debug_step, run_full_workflow), step config & analysis (update_step_config, harden_workflow, review_workflow_*), plan modification (add/update/delete step tools, todo_task routes, versioning), Goal Advisor proposal workflow, variables & MCP server config, schedule management (cron, modes, message authoring, optimizer best practices), shell, skills install/manage, and secrets two-step flow. Load when you need a tool's exact signature, parameters, or when-to-use rules and the inline cheat sheet doesn't suffice.", Modes: []string{"workshop"}},
	...
	"workshop-mode-flow":    {Group: "system", Description: "Workshop mode operating playbook: foundation checks (soul/soul.md objective + success criteria), the core run → eval → classify → act → verify loop, when to call harden_workflow vs create/apply a Goal Advisor plan-change proposal, optimization workflow steps, progressive hardening loop across groups, and the when-to-redirect-to-other-mode decision tree. Load when in workshop mode and choosing between harden / plan-change proposal / eval-improvement / no-action, or when running a multi-group hardening loop.", Modes: []string{"workshop"}},
	"debugging-flow":        {Group: "system", Description: "Debugging failed/stuck workflow steps: workshop investigation workflow (harden_workflow vs bounded plan change/proposal decision, manual fix tools), run-mode investigation workflow (query_step, debug_step, list_executions, read-only review), root cause → fix mapping table (description gaps, validation issues, context wiring), workshop vs run mode fix options. Load when a step fails or behaves unexpectedly, when the user asks 'why is it stuck' / 'what happened', or before deciding to retry vs fix the underlying step.", Modes: []string{"multi-agent", "workshop", "run"}},
...
func RegisterReferenceDocTool(agent *mcpagent.Agent, currentMode string, logger loggerv2.Logger) {
	desc := "Get the full reference documentation for a workshop concept. " +
		...
		"writing to db/kb/skill stores, running harden_workflow, or applying material plan changes. " +
