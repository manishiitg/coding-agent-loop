...
For requests to connect Slack, add a channel route, change a bot grant, block senders, or configure Slack triggers, load `builder-reference/references/slack-bot-routing.md`. Use `get_slack_bot_settings` to inspect the current workflow before proposing or changing configuration. The route tools are scoped to this workflow; shared credentials and enablement belong to operator setup in the UI. Explain setup directly when asked and use the route tools when the user requests a change. Never ask for bot/app tokens in chat.
