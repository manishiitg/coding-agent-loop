...
var allKinds = map[string]kindMeta{
	...
	"design-reporting-ui": {Group: "report", Description: "Design the reporting UI as one workflow-owned db/reports/index.html experience: live goal/evaluation/cost helpers, optional prebuilt metric widgets, report-owned approval buttons, and direct requests via window.report.sendChatMessage that reuse an existing workflow chat or create one only if needed.", Modes: []string{"workshop"}},
...
var referenceKinds = map[string]kindMeta{
	...
	"plan-drift-review":     {Group: "system", Description: "Plan Drift compatibility and prompt-quality review: inspect due candidates using current step-description guidance, reuse unchanged checks, apply bounded safe repairs, and persist results. Escalate only genuine decisions or platform boundaries; do not redesign unrelated architecture or require future runs to close applied fixes.", Modes: []string{"workshop"}},
	...
	"technical-review":      {Group: "system", Description: "Exception-driven QA and bounded workflow/scheduler repair; general optimization belongs to Architecture.", Modes: []string{"workshop"}},
	"architecture-review":   {Group: "system", Description: "Improve prompts, orchestration, schedule dependency/queue/runtime topology, stores, reports and efficiency; propose measurable changes without editing.", Modes: []string{"workshop"}},
	...
	"reporting-policy":        {Group: "system", Description: "HTML report contract: db/reports/index.html, goal/evaluation/cost data helpers and prebuilt widgets, live data and field writes, approval-save then direct sendChatMessage to an existing workflow chat or a new one only if needed, queued receipts, authoring, refresh, preview, and Run-mode boundaries. Load for reports, dashboards, report action buttons, agent chat handoffs, themes, or layouts.", Modes: []string{"workshop", "run"}},
