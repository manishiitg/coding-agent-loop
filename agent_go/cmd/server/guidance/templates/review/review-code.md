...
- **Browser + scripted fit:** if a browser-enabled or browser-heavy step is declared as `scripted` and has a saved `main.py`, flag this as at least WARNING and usually CRITICAL when the flow depends on live UI state, auth, dynamic selectors, pagination, or third-party page timing. The recommended default is `agentic` for browser/UI automation so the agent can adapt from fresh snapshots. Keep `scripted` only when the user explicitly asked for scripted browser execution AND the script uses durable selectors, state-driven waits, and has proven stable across 10+ scenario-covering successful runs.
