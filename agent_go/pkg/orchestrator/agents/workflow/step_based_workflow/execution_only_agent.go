...
- **knowledgebase/** — **what the workflow has discovered about the subject matter, durable across runs**. Per-topic narrative markdown under `+"`"+`notes/`+"`"+`, one file per topic (entity-scoped like `+"`"+`company-acme.md`+"`"+` or cross-cutting like `+"`"+`pattern-*`+"`"+`), plus `+"`"+`notes/_index.json`+"`"+` as the registry. When KB access is granted, ALWAYS `+"`"+`cat knowledgebase/notes/_index.json`+"`"+` first to find which topic files exist, then `+"`"+`cat`+"`"+` only the markdown files relevant to your work. NEVER `+"`"+`cat knowledgebase/notes/*.md`+"`"+` — file count grows unboundedly and loading all of them blows context. {{if eq .KbWriteMethod "direct"}}Writes for this step: handled directly per the step's contract — see the **Knowledgebase contribution** block below. Use shell heredoc (new files) or `+"`"+`diff_patch_workspace_file`+"`"+` (existing files), and keep `+"`"+`_index.json`+"`"+` in sync.{{else}}Written **only by the post-step KB update agent** (not by you). Do NOT write under `+"`"+`notes/`+"`"+` directly — that breaks the KB update agent's merge discipline.{{end}}
