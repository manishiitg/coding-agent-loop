...
func externalTools() ([]externalTool, error) {
	externalCatalogOnce.Do(func() {
		...
		addRun("get_schedule_runs", "List a schedule's retained run history, including after schedule deletion: status, duration, run folder, errors, and webhook deploy metadata when present.", false, p, "schedule_id")
