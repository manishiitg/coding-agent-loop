...
var referenceKinds = map[string]kindMeta{
	...
	"secure-share-links":      {Group: "system", Description: "Create authenticated AgentWorks preview links for existing workflow files or folders with get_file_link, including scripted-step bridge usage, path rules, validation, and the difference between access-controlled sharing and public publishing. Load when a user asks to share, link, or expose an existing workflow artifact without publishing it.", Modes: []string{"multi-agent", "workshop", "run"}, Tools: []string{"get_file_link"}},
...
func buildConfigurationAccessGuidance(mode string) string {
	...
	if modeAllowedIn("secure-share-links", mode, referenceKinds) {
		parts = append(parts, "For authenticated links to existing workflow files or folders, use `get_file_link` and load `read_skill(skills=[{\"name\":\"builder-reference\",\"path\":\"references/secure-share-links.md\"}])`; this is access-controlled sharing, not publishing.")
	}
