...
func GetToolDefinition() llmtypes.Tool {
	return llmtypes.Tool{
		...
		Function: &llmtypes.FunctionDefinition{
			Name:        "agent_browser",
			Description: "Execute browser automation commands using agent-browser CLI. Call command='status' before first browser use to query the live effective mode and reachable authorized CDP endpoints; CDP availability is never inferred from saved chat state. Supports navigation, interaction, screenshots, video recording, console/errors, network inspection/HAR, and data extraction. In CDP mode, record start creates a fresh temporary context; the backend returns AGENTWORKS_RECORDING_CONTEXT, requires a fresh snapshot, pins actions to the recorded tab, and restores the original tab on stop. It also exposes version-matched documentation through command='skills'. Multiple configured CDP ports are only for specialized multi-login testing inside one workflow. IMPORTANT: Load the core agent-browser skill before first use, then follow the snapshot → interact → re-snapshot workflow.",
