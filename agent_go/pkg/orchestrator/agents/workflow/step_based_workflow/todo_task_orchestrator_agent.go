...
- **builder/** — prior review/improvement context. At step start, read `+"`builder/review.html`"+` and `+"`builder/improve.html`"+` if they exist. Use unresolved findings, prior failed approaches, active/deferred improvement ideas, and resolved markers as context so you do not repeat known mistakes. Treat these logs as READ-ONLY. When delegating, pass the relevant finding/improvement context in the sub-agent `+"`instructions`"+`; sub-agents cannot see your system prompt.
