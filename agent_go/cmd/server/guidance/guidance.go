...
var referenceKinds = map[string]kindMeta{
	...
	"regular":                     {Group: "system", Description: "regular step design: use for coherent one-turn outcomes without a same-context proof/repair loop, or as the scripted boundary for deterministic work; covers anatomy (description, context flow, required validation_schema, store access), agentic vs scripted modes, and anti-patterns. Load before adding a regular step or when unsure which step type fits.", Modes: []string{"workshop"}},
