...
export function AnthropicSection({ config, onUpdate, onTestAPIKey, apiKeyStatus, apiKeyError, metadata }: AnthropicSectionProps) {
  ...
  const { availableAnthropicModels, saveLLM, testAPIKey: testAPIKeyFromStore, lockedProviders, llmConfigLocked } = useLLMStore()
  
  const isLocked = llmConfigLocked || lockedProviders.includes('anthropic')
            ...
            <div className="space-y-2">
              <div className="flex gap-2">
                <input
                  ...
                  placeholder={isLocked ? "••••••••••••••••" : "Enter your Anthropic API key"}
