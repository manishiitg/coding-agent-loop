...
var allKinds = map[string]kindMeta{
	...
	// Improvements — evidence-driven harden/replan flows
	...
	"improve-workflow":    {Group: "improve", Description: "Unified evidence-driven workflow improvement across plan, eval, report dashboard quality, KB, learnings, and db: harden, replan, or clean up evidence stores from run/eval evidence", Modes: []string{"workshop"}},
	"improve-evaluation":  {Group: "improve", Description: "Evaluation plan changes and coverage health checks", Modes: []string{"workshop"}},
	...
	"improve-report":      {Group: "report", Description: "Report dashboard goal tracking, accuracy, plan/issue surfacing, live-data wiring, layout, color, density, and responsive design improvements", Modes: []string{"workshop"}},
}
...
var referenceKinds = map[string]kindMeta{
	...
	"review-improve-log":  {Group: "system", Description: "The single workflow log spec: builder/improve.html format (newest-on-top timeline of run/monitor/artifact-review/decision/user-rule/note/open-finding entries, Bug/Goal plus cost/time prose tiles, per-run Artifact Review and LLM/model/cost/time readouts, monthly archive), a copy-paste starter HTML skeleton, the prose close-out ritual, and the one-time migration off old Markdown / F-/I- ids / improve-decision blocks. Load before writing the log from any /review-* or /improve-* skill or the monitor.", Modes: []string{"workshop", "run"}},
	"post-run-monitor":    {Group: "system", Description: "The post-run monitor playbook: read run evidence + changelog + eval files, form a Bug verdict (ran-right) and a Goal verdict (hits-success-criteria, gated on Bug being clean), call harden_workflow for real Bugs, then run separate report-only Artifact Review and LLM/model/cost/time steps, tune the existing Auto Improve cron only when the bounded cadence policy warrants it, back up the final state, publish, and send one compact run-summary notification while highlighting transitions. Update builder/improve.html (verdict pills, goal card, signal tiles, Artifact Review/cost/time notes, cadence decisions, a run row, and a Monitor/Note entry only if something's wrong or materially notable); the verdict pills + headline in improve.html are the notification signal. Triage is diagnosis/verdict only; fix/harden starts the canonical harden_workflow repair tool; Artifact Review and LLM/cost/time are report-only; cadence tuning uses update_schedule cron_expression only, no schedule schema changes. Load when running the post-run monitor pass.", Modes: []string{"workshop", "run"}},
	...
	"optimize-playbook":   {Group: "system", Description: "Optimizer deep-dive: harden vs replan decision tree, eval, and auto-improvement framework", Modes: []string{"workshop"}},
	...
	"report-plan":         {Group: "system", Description: "Report toolchain: a report is HTML document(s) registered in report_plan.json (no widget grammar, no other format). HTML reads db/db.sqlite live via the window.report API and renders its own visuals. Covers the window.report API, goal-tracking dashboard contract, good-document + design-quality + theming + responsive guide, tabs, per-report themes, validate/preview. Load before authoring or editing reports/report_plan.json.", Modes: []string{"workshop"}},
	...
	"memory-usage":          {Group: "system", Description: "Persistent cross-session memory: save_memory, recall_memory, enrich_memory tools; the user-model philosophy (what to save vs not save); storage layout and recall guidelines. Load when the user asks to remember something, references past work, or you need to consolidate chat history.", Modes: []string{"multi-agent"}},
	...
	"reporting-policy":      {Group: "system", Description: "Live frontend report viewer policy: a report is HTML document(s) registered in report_plan.json — no widget grammar, no other format; HTML reads db/db.sqlite live via window.report. Covers the 'create a report / dashboard' dispatch (author an HTML doc, author once + read live, never regenerate per run), tabbed sections for per-entity/routed workflows, 'No report yet' diagnosis, auto-refresh discipline, Run-mode boundary. Load when the user mentions reports, dashboards, themes, or layouts.", Modes: []string{"workshop", "run"}},
	...
	"planning-steps":        {Group: "system", Description: "Workshop plan composition: take-action-by-default discipline, step composition defaults (regular by default, message_sequence for same-context ordered turns, validation_schema required, forward-only context flow), step types overview (regular / todo_task / routing / human_input / message_sequence / orphan), fixed-branch routing via route_selections, and pointers to plan-design, per-step-type deep dives, and workflow-patterns. Load before adding/editing plan steps.", Modes: []string{"workshop"}},
	"workshop-mode-flow":    {Group: "system", Description: "Workshop mode operating playbook: foundation checks (soul/soul.md objective + success criteria), the core run → eval → classify → act → verify loop, when to call harden_workflow vs replan_workflow_from_results, optimization workflow steps, progressive hardening loop across groups, and the when-to-redirect-to-other-mode decision tree. Load when in workshop mode and choosing between harden / replan / eval-improvement / no-action, or when running a multi-group hardening loop.", Modes: []string{"workshop"}},
