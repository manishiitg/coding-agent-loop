...
export function BedrockSection({ config, onUpdate, onTestAPIKey, apiKeyStatus, apiKeyError, metadata }: BedrockSectionProps) {
  ...
  const { customBedrockModels, addCustomBedrockModel, removeCustomBedrockModel, availableBedrockModels, saveLLM, testAPIKey: testAPIKeyFromStore } = useLLMStore()
      ...
      <Card className="p-4">
        ...
        <div className="space-y-4">
          ...
          <div className="border-t border-border pt-4 space-y-3">
            ...
            {region && <div className="text-sm text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 p-2 rounded-md"><div className="flex items-center gap-2"><CheckCircle className="w-4 h-4" /><span>AWS region loaded from environment variables</span></div></div>}
            <div className="space-y-2">
              <label className="block text-sm font-medium text-muted-foreground">AWS Region</label>
              <select value={region} onChange={(e) => handleRegionChange(e.target.value)} className="w-full px-3 py-2 border border-border rounded-md bg-background text-foreground focus:ring-2 focus:ring-primary focus:border-primary">
                <option value="us-east-1">US East (N. Virginia)</option>
                <option value="us-west-2">US West (Oregon)</option>
                <option value="eu-west-1">Europe (Ireland)</option>
                <option value="ap-southeast-1">Asia Pacific (Singapore)</option>
              </select>
              <div className="text-xs text-muted-foreground">Uses AWS IAM roles for authentication. Make sure your AWS credentials are configured.</div>
            </div>
          </div>
