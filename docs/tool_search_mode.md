...
Tool Search Mode is implemented via mcpagent library options:

```go
// Enable tool search mode
mcpagent.WithToolSearchMode(true)

// Specify pre-discovered tools
mcpagent.WithPreDiscoveredTools([]string{"read_workspace_file", "write_workspace_file"})
```

These options are applied in `NewBaseAgent()` when creating agents:
...
| Scenario | Reason |
|----------|--------|
| **Large Tool Catalogs** | 50+ tools across multiple servers |
| **Diverse Workflows** | Steps need very different tool sets |
| **Token-Constrained Models** | Smaller context windows |
| **Exploratory Tasks** | Agent doesn't know which tools it needs upfront |

### Not Recommended

| Scenario | Reason |
|----------|--------|
| **Small Tool Sets** | < 20 tools, overhead not worth it |
| **Known Tool Requirements** | Steps always use the same specific tools |
| **Time-Critical Operations** | Tool search adds latency |
| **Simple Workflows** | Extra complexity not justified |

---

## Database Schema

Tool Search Mode adds two columns to the `preset_queries` table:
...
### PostgreSQL
```sql
ALTER TABLE preset_queries ADD COLUMN use_tool_search_mode BOOLEAN DEFAULT FALSE;
ALTER TABLE preset_queries ADD COLUMN pre_discovered_tools TEXT DEFAULT '[]';
```
...
{
  "use_tool_search_mode": true,
  "pre_discovered_tools": ["tool1", "tool2"]
}
```
...
- [Code Execution Mode](code_execution_mode.md) - Another agent mode for Go code execution
- [Step Config Specification](step_config_format_specification.md) - Full step config format
- [Workflow Orchestrator](workflow_orchestrator.md) - Overall orchestration system
- [Tool Filtering](tool_filtering_and_configuration.md) - Tool selection and configuration
- [Plan Tool Optimization](plan_opt_tool_optimization_agent.go) - Optimization agent logic
