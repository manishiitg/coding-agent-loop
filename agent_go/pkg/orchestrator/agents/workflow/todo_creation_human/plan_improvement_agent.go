...
func (pim *PlanImprovementManager) createPlanImprovementAgent(ctx context.Context, workspacePath string) (agents.OrchestratorAgent, error) {
	// Set folder guard paths: read-only access to runs/ folder and learnings/ folders
	...
	// Agent has read-only access to runs/ folder for execution results and both learnings/ folders for learnings analysis
	readPaths := []string{runsPath, learningsPath, learningCodeExecPath}
	writePaths := []string{} // No write access - read-only agent
	pim.SetWorkspacePathForFolderGuard(readPaths, writePaths)
	pim.GetLogger().Infof("📊 Setting folder guard for plan improvement agent - Read paths: %v, Write paths: %v (read-only access to runs/ and both learnings/ folders)", readPaths, writePaths)
...
func (pim *PlanImprovementManager) PlanImprovementOnly(ctx context.Context, workspacePath string) (string, error) {
	...
	allowedPaths := "['runs/', 'learnings/', 'learning_code_exec/']"
...
- **Restricted Access**: You ONLY have access to these subdirectories: ` + templateVars["AllowedPaths"] + `
   ...
   - Always start listing from the allowed subdirectories (e.g., folder="runs", folder="learnings", or folder="learning_code_exec").
