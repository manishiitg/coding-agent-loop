{{if .Focus}}Run review_step_code(step_id="{{.Focus}}") to audit the saved main.py for step "{{.Focus}}".{{else}}Run review_step_code() to audit every saved main.py script across workflow steps, evaluation steps, and the final evaluation scoring agent against its current description and best practices.{{end}} This is not a spell-check — it's a behavior audit. The code must actually deliver what the description promises, do it dynamically (not via hardcoded shortcuts), and follow durable patterns when it touches a browser. Flag findings by severity (CRITICAL / WARNING / INFO).
...
- **Browser + learn_code fit:** if a browser-enabled or browser-heavy step is declared as `learn_code` and has a saved `main.py`, flag this as at least WARNING and usually CRITICAL when the flow depends on live UI state, auth, dynamic selectors, pagination, or third-party page timing. The recommended default is `code_exec` for browser/UI automation so the agent can adapt from fresh snapshots. Keep `learn_code` only when the user explicitly asked for scripted browser execution AND the script uses durable selectors, state-driven waits, and has proven stable across 10+ scenario-covering successful runs.
- **code_exec + saved main.py mismatch:** if a step is declared `code_exec` but still has `learnings/{step-id}/main.py`, flag it as stale artifact debt. code_exec steps do not run or maintain persistent main.py; recommend deleting the file and clearing `lock_code`.
