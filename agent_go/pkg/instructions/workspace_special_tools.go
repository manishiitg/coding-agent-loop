...
- ` + "`list_llm_capabilities(capability?, include_models?)`" + ` — Inspect which providers/models are supported and currently usable for ` + "`chat`" + `, ` + "`read_image`" + `, ` + "`generate_image`" + `, ` + "`generate_video`" + `, ` + "`text_to_speech`" + `, ` + "`speech_to_text`" + `, and ` + "`generate_music`" + `. Use this before choosing a provider when the user's request depends on provider capability, auth, pricing, or runtime availability.
...
- ` + "`search_web_llm(query, provider, model_id?)`" + ` — Run a live web search using a published search-capable provider from ` + "`config/published-llms.json`" + `. ` + "`provider`" + ` is required. ` + "`model_id`" + ` is optional; omit it unless the user requested a specific published model, and the backend will pick a working search-capable model for that provider.
...
- ` + "`read_image(filepath, query)`" + ` — Analyze an image file using ` + "`config/image-analysis-config.json`" + `. If no image-analysis config exists, it falls back to the current chat model. ` + "`codex-cli`" + ` and ` + "`claude-code`" + ` are supported by passing the local workspace image path to the CLI.
