...
func CreateHumanTools() []llmtypes.Tool {
	...
	humanTools = append(humanTools, createPublishPulseUpdateTool())
...
func WorkshopHumanToolNames() []string {
	return []string{"human_feedback", "notify_user", "publish_pulse_update", "google_workspace_cli", "get_human_input_request", "list_approved_fixer_decisions", "create_human_input_request", "answer_human_input_request", "mark_human_input_consumed", "dismiss_duplicate_human_input_request"}
}
...
func notificationSummaryFromArgs(
	...
	reviews, err := pulseReviewsFromArg(args["pulse_reviews"])
	if err != nil {
		return nil, err
	}
