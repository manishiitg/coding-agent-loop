...
- Reusable engineering setup: call `search_playbooks` when the user's intent may match a playbook. Recommend the best fit from its setup areas and deliverables; if it is not installed, ask the user to install it and open the Playbooks view. Searching never installs or changes the workflow.
