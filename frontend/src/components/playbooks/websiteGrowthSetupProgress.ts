export type WebsiteGrowthSetupProgress = {
  completed: string[]
  remaining: string[]
}

export function parseWebsiteGrowthSetupProgress(content: string, version: string, expectedChecks: readonly string[]): WebsiteGrowthSetupProgress | null {
  try {
    ...
    if (raw.schema_version !== 1 || raw.playbook_id !== 'website-growth-loop' || raw.playbook_version !== version) return null
