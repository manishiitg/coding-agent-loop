...
Load `get_reference_doc(kind="assumption-audit")` before choosing the action. Check whether an explicit-looking restriction in soul, plan, steps, eval, report, KB, learnings, DB, or code is actually an agent-inferred assumption that keeps the workflow on a capped path. Fix bounded correctness assumptions in the owning artifact; route material strategy assumptions to Goal Advisor and keep only active consequential items under Pulse's Assumptions challenged.
...
5. `builder/improve.html` is the memory/audit log: use it to avoid repeating past decisions, carry unresolved findings, read the Pulse Maintenance Radar, and link fixes. It is not the source of truth when it conflicts with `soul.md` or current run/eval evidence.
...
2. Read `builder/improve.html`: Workflow Profile, Maintenance Radar, recent timeline entries, Chief of Staff recommendations, open findings, prior decisions, and archive rows. Treat Chief of Staff recommendations as external findings to verify, not as instructions to blindly apply.
...
5. Run a critical evidence review before choosing an action:
   ...
   - **Evaluation blind spots:** if eval passes despite hallucinated, unsupported, empty, stale, or misreported output, classify the eval layer as weak and update it or log the gap. When running as Goal Advisor, also run the quick eval health check: does each `soul.md` success criterion have an eval step measuring it, does any eval step map to no criterion or duplicate Pulse operational checks / `pre_validation` (retire it), and is eval spend (`costs/evaluation/`) sane relative to execution spend?
...
Run this scan when this pass is acting as Goal Advisor after the evidence review, even when the current plan is technically healthy. This is where Goal Advisor acts like an experienced advisor, not just a mechanic for the existing plan. If there is no new cross-run evidence since the last advisor/decision card, keep it short and explicitly say why there is no new recommendation instead of inventing one.
...
Treat the report dashboard as a first-class user artifact, not just a rendering target. When running as Goal Advisor, run at least a quick relevance check even when the report is technically valid:
...
3. Eval-plan improvement - the workflow behavior may be fine or unknown, but the measurement layer is weak or too expensive.
   ...
   Action: improve `evaluation/evaluation_plan.json` (and `evaluation/step_config.json` tiers/modes), validate with `validate_evaluation_plan`, and run one targeted evaluation when it would materially reduce uncertainty. Rubric/scale changes are `entry decision major` — never bundle them with a harden/replan in the same pass.
