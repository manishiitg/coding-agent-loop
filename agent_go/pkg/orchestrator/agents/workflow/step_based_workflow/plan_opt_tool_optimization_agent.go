...
// WorkflowPlanToolOptimizationTemplate holds template variables for tool optimization prompts
type WorkflowPlanToolOptimizationTemplate struct {
...
// WorkflowPlanToolOptimizationAgent optimizes tool selections in step_config.json
type WorkflowPlanToolOptimizationAgent struct {
	*agents.BaseOrchestratorAgent
	baseOrchestrator *orchestrator.BaseOrchestrator // Reference to base orchestrator for RequestHumanFeedback
}

// NewWorkflowPlanToolOptimizationAgent creates a new plan tool optimization agent
func NewWorkflowPlanToolOptimizationAgent(config *agents.OrchestratorAgentConfig, logger loggerv2.Logger, tracer observability.Tracer, eventBridge mcpagent.AgentEventListener, baseOrchestrator *orchestrator.BaseOrchestrator) *WorkflowPlanToolOptimizationAgent {
	...
	return &WorkflowPlanToolOptimizationAgent{
...
func (ptom *PlanToolOptimizationManager) createPlanToolOptimizationAgent(ctx context.Context, workspacePath string) (agents.OrchestratorAgent, error) {
	...
	createAgentFunc := func(cfg *agents.OrchestratorAgentConfig, logger loggerv2.Logger, tracer observability.Tracer, eventBridge mcpagent.AgentEventListener) agents.OrchestratorAgent {
		return NewWorkflowPlanToolOptimizationAgent(cfg, logger, tracer, eventBridge, ptom.BaseOrchestrator)
	}
...
func (ptom *PlanToolOptimizationManager) PlanToolOptimizationOnly(ctx context.Context, workspacePath string, stepID string) (string, error) {
	...
	baseAgent := toolOptimizationAgent.(*WorkflowPlanToolOptimizationAgent).GetBaseAgent()
	if baseAgent == nil {
...
func (agent *WorkflowPlanToolOptimizationAgent) Execute(ctx context.Context, templateVars map[string]string, conversationHistory []llmtypes.MessageContent) (string, []llmtypes.MessageContent, error) {
	...
	templateData := WorkflowPlanToolOptimizationTemplate{
		WorkspacePath:          workspacePath,
		PlanJSON:               planJSON,
		StepConfigJSON:         stepConfigJSON,
		CurrentToolConfigsJSON: currentToolConfigsJSON,
...
func (agent *WorkflowPlanToolOptimizationAgent) toolOptimizationSystemPromptProcessor(templateVars map[string]string) string {
...
func (agent *WorkflowPlanToolOptimizationAgent) toolOptimizationUserMessageProcessor(templateVars map[string]string) string {
