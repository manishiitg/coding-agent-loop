Design the workflow's reporting UI from the ground up. **Load `get_reference_doc(kind="report-plan")` first and follow it** — it is the canonical report model (a report is HTML document(s) in `reports/report_plan.json` reading `db/db.sqlite` live via `window.report`, no widget grammar) plus the content guide, design-quality bar, theming, and responsive rules. Also load `get_reference_doc(kind="html-output")` for the layout + dark-mode skeleton to start from. This skill is the *process*; that reference is the *substance*.{{if .Focus}}
