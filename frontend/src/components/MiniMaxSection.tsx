...
export function MiniMaxSection({ config, onUpdate, onTestAPIKey, apiKeyStatus, apiKeyError, metadata }: MiniMaxSectionProps) {
  const [apiKey, setApiKey] = useState(config.api_key || '')
  const [isPublishing, setIsPublishing] = useState(false)
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [publishName, setPublishName] = useState('')
  const [publishError, setPublishError] = useState<string | null>(null)
  const { availableMinimaxModels, saveLLM, testAPIKey: testAPIKeyFromStore, lockedProviders, llmConfigLocked } = useLLMStore()

  const isLocked = llmConfigLocked || lockedProviders.includes('minimax')
  ...
  const handlePublishToLibrary = async () => {
    ...
    if (!apiKey.trim()) {
      setPublishError('API key is required to publish MiniMax models')
      return
    }
    ...
    try {
      const optionsWithTemp = config.temperature !== undefined
        ...
        : config.options
      const testResult = await testAPIKeyFromStore('minimax', apiKey, config.model_id, optionsWithTemp)
  ...
  const modelsFromMetadata = metadata?.filter(m => m.provider === 'minimax').map(m => m.model_id) || []
  const allModels = Array.from(new Set([...modelsFromMetadata, ...availableMinimaxModels]))
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-foreground">MiniMax Configuration</h3>
        {isLocked && (
            ...
            <div className="space-y-2">
              <div className="flex gap-2">
                <input
                  ...
                  placeholder={isLocked ? "••••••••••••••••" : "Enter your MiniMax API key"}
