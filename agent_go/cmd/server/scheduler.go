...
func postRunMonitorGateStep(pulseRunID, runFolder, runStatus string) postRunMonitorStep {
	return postRunMonitorStep{
		...
		query: fmt.Sprintf("STEP 1 — PULSE GATE / WORKLIST. This is the only mandatory intelligence step for this Pulse run. pulse_run_id=%q, run_folder=%q, run_status=%q.\n\n"+
			"Use a progressive evidence scan, not a full audit. First read builder/improve.html (authoritative history/current cadence), including any .advisor-experiment card and its data-status/data-review-after, get_pulse_module_state(workspace_path=\"<current workflow>\"), latest run metadata/summary, changelog filenames and review flags, existence/freshness metadata for plan/eval/report artifacts, db/README or a compact schema summary, per-step learning metadata, a compact KB note index, open/answered report_human_inputs, and pending Chief of Staff recommendation cards. Do not load full report HTML, full KB/learnings, broad DB rows, long logs, or every cost file unless a compact signal makes that module plausibly due or you need one targeted fact to justify its decision. The selected module performs the deep audit later; Gate only chooses the evidence-backed worklist and cadence. If the scan finds a plausible bug signal, mark Harden due so Harden can investigate, fix, and verify it. Do not treat the SQLite module cache as historical truth.\n\n"+
			...
			"Be agentic: decide what is due from workflow evidence, not a hardcoded cadence. Every skipped module must include a reason, evidence, and at least one concrete next-check condition: next_check_at, a positive cooldown_runs, or next_check_after_run_id. Write that next check visibly in the Gate/Worklist entry so the user can understand it; SQLite only mirrors it for scheduler/popup state. Evidence can override an earlier cooldown, but if you check a module earlier than previously planned, state what new evidence caused the override. High-frequency workflows should normally roll up cost/time checks across several runs instead of running cost_llm_time every time; mark it due immediately for missing/unpriced telemetry, a material cost or latency change, a model/tier change, or when its planned next check arrives. Final dashboard, backup, publish, and notify happen later in one ordered finalizer turn.\n\n"+
			"Module meanings:\n"+
			"- harden: due for real Bug findings, failed producing steps, stale guards, selector/API/runtime breakage, workflow code that reuses an older receipt/artifact, broken report contracts that block truth, or CoS recs that are operational bugs.\n"+
...
func postRunMonitorModuleSteps(pulseRunID string) []postRunMonitorModuleStep {
	steps := []postRunMonitorModuleStep{
		...
		{pulseModuleGoalAdvisor, postRunMonitorStep{"goal-advisor", fmt.Sprintf("PULSE MODULE — GOAL ADVISOR. pulse_run_id=%q. Run only the strategy advisor module selected by Pulse Gate. Do not perform the strategic review inline. Read only the Gate/worklist and the active .advisor-experiment card, if any, enough to pass a concise focus, then call run_goal_advisor_review(pulse_run_id=%q, focus=\"Pulse-selected Goal Advisor module; use Gate evidence; choose recovery, healthy 10x/headroom, active-experiment measurement, or approved-answer mode. Preserve one active advisor experiment in builder/improve.html and advance it instead of creating duplicates. Use the 10x counterfactual as a thinking lens, not an unsupported promise. Act as an expert strategy advisor, not routine hardening or eval correctness. Challenge consequential agent-inferred assumptions and architecture that may cap the goal; treat only explicit user-approved boundaries as durable soul constraints. Keep active assumptions in the top Assumptions challenged area. Never propose/ask about stale receipts, current-run binding, paths, parsing, schemas, or fail-closed behavior; hand those to Eval Health/Harden.\"). Capture the returned execution_id. Do not wait by polling: no sleep loops, no repeated list_executions, and no repeated query_step. Use query_step(step_id=\"goal-advisor\", execution_id=\"<returned execution_id>\") at most once for an immediate status/result check; if still running, stop and rely on [AUTO-NOTIFICATION] to resume when the background Goal Advisor completes. Do not call harden_workflow, improve_kb, improve_learnings, or improve_db from this parent Pulse turn. When the result is available from [AUTO-NOTIFICATION] or a one-off query_step, update/confirm the visible Goal Advisor experiment/outcome in builder/improve.html if needed and finish by calling mark_pulse_module_result(workspace_path=\"<current workflow>\", pulse_run_id=%q, module=\"goal_advisor\", result=\"done|changed|blocked|failed|skipped\", reason=\"...\", evidence=[...]). Then stop.", pulseRunID, pulseRunID, pulseRunID)}},
	}
...
func postRunMonitorFinalSteps(pulseRunID string) []postRunMonitorStep {
	return []postRunMonitorStep{
		{"finalize", fmt.Sprintf("PULSE FINALIZER. pulse_run_id=%q. Complete the four final commands below in order in this ONE turn. Do not split them into more chat turns. Before each command call mark_pulse_final_command_result(workspace_path=\"<current workflow>\", pulse_run_id=%q, command=\"dashboard|backup|publish|notify\", status=\"running\", reason=\"...\"); immediately after it finishes call the same tool with done, skipped, blocked, or failed and a factual one-sentence reason. Use the actual single command name, not the pipe-separated example. Continue to Notify even when an earlier command fails, and report the failure honestly.\n\n"+
			...
			"2. BACKUP. Mark backup running. Read workflow.json.backup, backup/status.json, and get_reference_doc(kind=\"backup-strategy\"). Back up the final Gate + selected-module + dashboard state. If backup is disabled, set up the zero-config local-git default. Skip the actual backup only when the status/source-hash check proves this exact current state is already backed up; mark skipped with that reason. Otherwise perform it, always update backup/status.json, update the latest run row with the result, and mark done or failed.\n\n"+
