...
- **Writing a GOOD report document (md/html).** Mechanics above place content; these make it readable — instruct the step that generates the doc accordingly:
  ...
  - **SELF-REVIEW the design visually — don't ship HTML you haven't looked at.** You cannot judge a visual design by reading its source; render it and look. Loop:
    1. Write a **self-contained** copy of the report HTML to a temp path (e.g. `db/reports/_review.html`) with **data baked in** — inline the actual numbers/rows (run the queries yourself) and base64 or omit images. `window.report` does NOT exist when a file is opened outside the app, so a report that depends on it renders blank in the browser; the review copy must stand alone.
    2. Open it in `agent_browser` (navigate to the file) and `browser_take_screenshot` — capture at desktop and at ~480px (resize the viewport) to check responsiveness.
    3. `read_image` the screenshot and **critique against the design bar above** — hierarchy, spacing, contrast (light AND dark — screenshot both by toggling a `.dark` class on `<html>`), alignment, no clipping/overflow, chart legibility.
    4. Fix the real report HTML and repeat until it looks genuinely polished. Then delete the temp review file.

    Do this whenever you author or substantially change an HTML report — a quick screenshot-and-look catches the cramped spacing / broken dark mode / overflowing table that source review misses.
