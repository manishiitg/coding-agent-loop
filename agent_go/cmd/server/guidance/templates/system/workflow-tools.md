...
- **Goal Advisor plan-change proposals** — Material strategy/path changes use the existing report interaction flow, not a separate replan tool. In scheduled Pulse/Goal Advisor, create or refresh `create_human_input_request(source="goal_advisor", input_id="plan-proposal-...", options=[approve,reject,defer], context="<proposal + exact intended edits + rationale + expected impact + risk + evidence>")`. A later Pulse run may apply an approved proposal with normal plan modification/config/eval/report tools and then call `mark_human_input_consumed`. In active manual workshop chat, apply a bounded evidence-backed plan change directly only when the user is asking for improvement and the evidence is strong.
