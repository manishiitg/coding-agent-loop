...
// The per-user ("yours") secret store is gone: only workflow-box secrets and
// globals exist. Guidance must never name the removed tools, or the builder
...
func TestSecretGuidanceNamesNoRemovedTools(t *testing.T) {
	...
	err = fs.WalkDir(productConfigFiles, "prompts", func(path string, d fs.DirEntry, err error) error {
