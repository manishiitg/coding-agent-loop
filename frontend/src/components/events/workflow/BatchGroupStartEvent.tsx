...
export const BatchGroupStartEvent: React.FC<BatchGroupStartEventProps> = ({ event, compact = false }) => {
  ...
  return (
    <div className="border-l-2 border-blue-500 pl-3 py-3 bg-blue-50/30 dark:bg-blue-900/20 rounded-r-md text-sm">
      <div className="flex items-center gap-2 mb-2">
        <div className="p-1 bg-blue-100 dark:bg-blue-800/60 rounded-md">
          <Play className="w-3.5 h-3.5 text-blue-600 dark:text-blue-300" />
        </div>
        <div className="flex-1">
          <div className="font-bold text-blue-900 dark:text-blue-100 flex items-center gap-2">
            Batch Group Started
            <span className="text-[10px] font-normal bg-blue-100 dark:bg-blue-800/60 px-1.5 py-0.5 rounded text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-700/50 font-mono">
      ...
      {event.variable_values && Object.keys(event.variable_values).length > 0 && (
        <div className="mt-2 text-xs text-gray-600 dark:text-gray-300 bg-white/50 dark:bg-gray-800/40 p-2.5 rounded border border-blue-100 dark:border-blue-800/30">
          ...
          <div className="space-y-1">
            {Object.entries(event.variable_values).map(([key, value]) => (
              <div key={key} className="flex items-start gap-2 text-[11px]">
                <span className="font-mono text-blue-800 dark:text-blue-200 font-semibold min-w-[80px]">{key}:</span>
                <span className="text-blue-700 dark:text-blue-300 break-words flex-1 bg-blue-50/50 dark:bg-blue-900/20 px-1 rounded">
                  {value || '(empty)'}
                </span>
              </div>
            ))}
      ...
      {/* Run Folder */}
      {event.run_folder && (
        <div className="mt-2 flex items-center gap-1.5 text-[10px] text-blue-500 dark:text-blue-400 opacity-80 pl-1">
