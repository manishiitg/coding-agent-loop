...
var allKinds = map[string]kindMeta{
	...
	"ops-review":            {Group: "review", Description: "Focused read-only Technical Review: diagnose concrete correctness, reliability, validation, store, report, evaluation, scheduler, or safety failures", Modes: []string{"workshop"}},
	...
	"design-reporting-ui": {Group: "report", Description: "Design one or more workflow-owned db/reports/*.html views with shared toolbar discovery: live goal/evaluation/cost helpers, optional prebuilt metric widgets and daisyUI, report-owned approval buttons, and direct requests via window.report.sendChatMessage.", Modes: []string{"workshop"}},
	"improve-report":      {Group: "report", Description: "Read-only report dashboard accuracy, goal/evaluation/cost semantics, shared metric widgets, live-data, layout, and responsive-design review", Modes: []string{"workshop"}},
}
