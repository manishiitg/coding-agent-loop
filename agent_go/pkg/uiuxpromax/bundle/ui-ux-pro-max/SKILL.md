...
This skill is optional design intelligence, not a mandatory framework or a replacement for the active product contract. Choose the CSS framework, component library, charting library, and implementation style that best fit the requested outcome and the runtime. For workflow reports, `builder-reference/references/reporting-policy.md` and `builder-reference/references/html-output.md` remain authoritative for host APIs and validation; use this skill for visual and interaction decisions. Do not create or persist a design system unless the task benefits from one. Treat every catalog result as a recommendation and keep the user's explicit direction above it.
...
The search script is projected into the current coding provider's skill directory. Locate it once for the shell command instead of assuming one provider-specific path:

```bash
UI_UX_SEARCH="$(find .claude/skills .cursor/skills .agents/skills .pi/skills -path '*/ui-ux-pro-max/scripts/search.py' -print -quit 2>/dev/null)"
test -n "$UI_UX_SEARCH" && python -B "$UI_UX_SEARCH" "<query>" --domain <domain>
```

If the skill is available only through `read_skill` and no projected script exists, use its bundled references directly and do not claim a catalog search ran.
...
### Step 2: Generate a design system when useful

Use `--design-system` when the task needs a coherent product-wide visual direction:

```bash
python -B "$UI_UX_SEARCH" "<product_type> <industry> <keywords>" --design-system [-p "Project Name"]
```
...
**Example:**
```bash
python -B "$UI_UX_SEARCH" "beauty spa wellness service" --design-system -p "Serenity Spa"
```
...
```bash
python -B "$UI_UX_SEARCH" "<query>" --design-system --persist -p "Project Name" --output-dir "<project-root>"
```
...
```bash
python -B "$UI_UX_SEARCH" "<query>" --design-system --variance <1-10> --motion <1-10> --density <1-10>
```
...
**Example:**
```bash
python -B "$UI_UX_SEARCH" "internal analytics dashboard" --design-system --variance 8 --motion 7 --density 8 -p "Ops Console"
```

### Step 3: Supplement with Detailed Searches (as needed)

```bash
python -B "$UI_UX_SEARCH" "<keyword>" --domain <domain> [-n <max_results>]
```
...
```bash
python -B "$UI_UX_SEARCH" "<keyword>" --stack <stack>
```
...
python -B "$UI_UX_SEARCH" "AI search tool modern minimal" --design-system -p "AI Search"

# Step 3: supplement
python -B "$UI_UX_SEARCH" "keyboard focus modal" --domain ux

# Step 4: stack guidelines
python -B "$UI_UX_SEARCH" "suspense streaming bundle" --stack nextjs
```
