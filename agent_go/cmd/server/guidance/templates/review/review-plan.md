...
2. **Learning audit**:
   ...
   - Flag any `code_exec` step with `learnings/{step-id}/main.py`; code_exec does not run or maintain persistent main.py, so the file is stale and should be deleted.
   - Flag learning writes for plumbing steps with no reusable HOW-to-run knowledge.
   - Flag browser-based steps using `learn_code`; browser work should stay agentic/code_exec unless there is an explicit user request plus a defensible 10+ run scenario-coverage exception.
   - Flag `learn_code` unless all three gates are satisfied: user explicitly asked for it, the step is highly deterministic, and 10+ successful runs cover the relevant scenarios/groups with eval/metric evidence at target. Also flag it while recent metric evidence is inconclusive or while the step behavior is still changing.
