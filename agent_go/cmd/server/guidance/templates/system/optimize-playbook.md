...
**Promotion rule:** Builder-created steps arrive as `agentic`, and you should never auto-promote to `scripted` on your own initiative — that judgment is the user's. On an **explicit user request**, set `scripted` right away so they can build and test the script (don't gate that on a run count — they can't accumulate runs without it). Treat 10+ scenario-covering successful runs (with eval/metric evidence at target) as the bar for **trusting the saved script as the stable fast path and freezing it with `lock_code`** — not as a precondition for the user creating a scripted step. Keep `lock_code=false` until that evidence exists even when the mode is already `scripted`, so the repair loop can still fix drift. Absent a user ask, keep `agentic` and improve descriptions, validation, learnings, or tool usage instead.
