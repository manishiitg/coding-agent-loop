...
type AgentConfigs struct {
	...
	KnowledgebaseWriteMethod     string          `json:"knowledgebase_write_method,omitempty"`      // "agent" | "direct". How KB writes happen when knowledgebase_access permits them. "agent" (default): the post-step KB update agent reads the step's trail + knowledgebase_contribution and merges into graph.json/notes/. "direct": the step agent writes directly via kb_upsert_entity / kb_upsert_relationship tools plus shell writes under notes/; no post-step agent runs for this step. Only meaningful when knowledgebase_access ∈ {"write", "read-write"}.
	KnowledgebaseContributionType string         `json:"knowledgebase_contribution_type,omitempty"` // "graph_only" | "notes_only" | "both". Scopes direct-mode KB writes to a subset of surfaces: "graph_only" registers kb_upsert_* but not notes writes; "notes_only" gives shell access to notes/ but skips kb_upsert_* registration; "both" (default) enables the full direct-write capability. Only consulted when knowledgebase_write_method == "direct"; agent-mode KB extraction is unaffected.
	KnowledgebaseContribution    string          `json:"knowledgebase_contribution,omitempty"`      // User-authored instruction for KB writes — what this step should contribute to graph.json / notes/. In "agent" write-method, it's the extraction instruction handed to the post-step KB update agent. In "direct" write-method, it's injected into the step agent's prompt as its contribution contract. Required to trigger KB writes; if empty, no KB write happens regardless of access.
