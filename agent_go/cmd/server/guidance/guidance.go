...
var referenceKinds = map[string]kindMeta{
	...
	"report-plan":         {Group: "system", Description: "Report toolchain: a report is HTML/Markdown document(s) registered in report_plan.json (no widget grammar). HTML reads db/db.sqlite live via the window.report API and renders its own visuals; markdown is static. Covers the window.report API, the good-document + design-quality + theming + responsive guide, tabs, per-report themes, validate/preview. Load before authoring or editing reports/report_plan.json.", Modes: []string{"workshop"}},
	...
	"reporting-policy":      {Group: "system", Description: "Live frontend report viewer policy: a report is HTML (primary; live data via window.report) or Markdown (secondary; static) document(s) registered in report_plan.json — no widget grammar. Covers the 'create a report / dashboard' dispatch (author an HTML doc, author once + read live, never regenerate per run), tabbed sections for per-entity/routed workflows, 'No report yet' diagnosis, auto-refresh discipline, Run-mode boundary. Load when the user mentions reports, dashboards, themes, or layouts.", Modes: []string{"workshop", "run"}},
	...
	"planning-steps":        {Group: "system", Description: "Workshop plan composition: present-plan-and-confirm discipline, step composition defaults (regular by default, validation_schema required, forward-only context flow), step types overview (regular / todo_task / routing / human_input / message_sequence / orphan), and pointers to plan-design (8-step walkthrough), per-step-type deep dives, and workflow-patterns. Load before adding/editing plan steps.", Modes: []string{"workshop"}},
