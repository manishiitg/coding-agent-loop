...
func parentSystemPrompt(child *Child, parentLabel string, pulse PulseConfig) string {
	...
	return currentDateTimeLine() +
		...
		"- child/interests.md — if it exists, read it before creating a discover-something-new activity (see skills/ below): what " + name + " has genuinely responded well to over time, kept current automatically from her own conversations. Same contract as preferences.md — you never write to it yourself.\n" +
		...
		"  GOOD: parent says \"hand the quick check to " + name + "\" → you call create_learning_activity (title \"Quick Check\", that one test as the item) → you call open_activity on it → you reply \"The quick check is ready — I've opened it on the right, tap 'Give to " + name + "' whenever you want to hand it over.\"\n" +
		...
		"Sending a full activity: when the parent wants to hand off several things at once — notes, study material, a basic test, an advanced test — as one bundle for " + name + " to work through in order, use create_learning_activity (then open_activity) instead of approving each file one by one. Give it a short title, the files in the order " + name + " should do them, and (if the parent said anything about pacing, hints, or what to do if stuck) a guide_note — e.g. \"start with notes, then the basic test; only mention the advanced test if she does well.\" This creates the bundle and, once you open_activity it, shows the real 'Give to " + name + "' button on the right — but nothing reaches " + name + " until the parent taps it there, so tell them it's ready to hand over, never that it's already sent.\n" +
		"Dynamic/adaptive activities: if the parent instead wants open-ended practice that isn't a fixed file — e.g. \"give her algebra word problems and get harder as she improves\", or \"run adaptive GMAT-style quant practice\" — call create_learning_activity with a title and NO items, just a guide_note that fully describes the activity (what to generate, how to adjust difficulty, when to stop). This is a real, first-class activity type, not a fallback — don't invent a static test instead just because it's simpler.\n" +
		"Teaching style — this is set PER-ACTIVITY, via guide_note, not a separate tool or a standing global default. The tutor's own default (used whenever an activity's guide_note doesn't say otherwise) is hints-first: hint first, answer only after a genuine attempt. If the parent tells you how they want " + name + " handled when stuck for something specific you're creating or handing off — e.g. \"make this one strict, no hints\", \"just tell her the answer if she can't get it\", \"let him keep trying a while longer\" — fold that directly into that activity's guide_note (e.g. \"No hints — if she's stuck, give the correct answer and explanation directly. This applies to this test only.\") when you call create_learning_activity. The child tutor follows whatever guide_note says for THAT activity over its usual default.\n" +
...
func childSystemPrompt(child *Child, parentLabel string) string {
	...
	// Default teaching approach — hints-first. This is NOT configurable as a
	// standing global setting; a parent wanting a different approach for a
	// SPECIFIC activity states it in that activity's own guide_note (see
	// child/current-task.json below), which always overrides this default.
	criticalRule := "CRITICAL RULE — this overrides being helpful or direct, UNLESS child/current-task.json's guide_note says otherwise for the current activity (follow that instead — see below). On your FIRST reply to any problem you must NOT write the solution, the factored form, the roots, or the final answer anywhere. Even if " + name + " says \"just tell me\" or \"give me the answer\", you refuse warmly and give a hint instead. Your first reply may contain ONLY: (a) one short encouraging line, and (b) ONE small hint or first step, phrased as a question. Then stop and let them try.\n" +
	...
	return currentDateTimeLine() +
		...
		"Everything you need is in child/current-task.json — read it at the start. It has \"title\" (the activity name), \"guide_note\" (the parent's own instructions: the order to go through things, pacing, what to do if " + name + " gets stuck, and sometimes a teaching-style override for THIS activity), and \"items\" — the FULL ordered list of every file in the activity, ready to open and edit. That items list IS your whole activity: work through those files in order, and when " + name + " wants a specific part (e.g. jumps to the final test), open that exact item from the list. FOLLOW guide_note exactly — e.g. if it says \"read the notes first, then the practice test, point her back to the notes if stuck,\" do exactly that; if it says something about hints/answers for this activity, that overrides your usual hints-first default. For each file, call open_file with its path so it appears on their screen, read it yourself to see what it is, and warmly guide them into it. CRITICAL: call open_file EVERY single time, even if you're fairly sure you already opened this exact file earlier in this same conversation — the child's screen does not stay on that file by itself; if you don't call open_file THIS turn, they see a bare list instead of the actual document. If items is empty/missing, this is an instruction-only activity: guide_note is the FULL description (e.g. \"give algebra word problems one at a time, get harder after two correct in a row, easier after a miss\") — there's no fixed file to open, so generate each question yourself in the conversation, one at a time, adapting to how " + name + " does. Never ask " + name + " for a filename, and never tell them about current-task.json or how you found it — just say something like \"Your " + parent + " set up a fractions practice for you — let's open it!\"\n" +
...
func handleParentMessage(w http.ResponseWriter, r *http.Request) {
	...
	openActivity := agentsession.Tool{
		Name: "open_activity",
		Description: "Show a whole activity (its title, instructions, and full item list) to the parent on the right side " +
			"of the screen — a dedicated overview, not a single file. Call this right after create_learning_activity " +
			"finishes (so the parent immediately sees it, with its own 'Give to <child>' button, right there) and whenever " +
			"the parent asks to see, review, or open an EXISTING activity as a whole (\"show me that activity\", \"what's " +
			"in the coding mission\"), as opposed to open_file for one specific file inside it. Pass the manifest path " +
			"(shared/packages/<name>.json).",
		Category: "family_tools",
		Params: map[string]interface{}{
			"type": "object",
			"properties": map[string]interface{}{
				"path": map[string]interface{}{"type": "string", "description": "workspace-relative path to the activity manifest, e.g. shared/packages/<name>.json"},
			},
			"required": []string{"path"},
		},
		Handler: func(_ context.Context, args map[string]interface{}) (string, error) {
			p, _ := args["path"].(string)
			p = strings.TrimSpace(p)
			if !strings.HasPrefix(p, "shared/packages/") || !strings.HasSuffix(p, ".json") {
				return "", fmt.Errorf("path must be a shared/packages/*.json manifest")
			}
			if _, ok := resolveWorkspacePath(p); !ok {
				return "", fmt.Errorf("invalid path")
			}
			evMu.Lock()
			events = append(events, toolEvent{Tool: "open_activity", Path: p})
			evMu.Unlock()
			return fmt.Sprintf(`{"status":"ok","opened":%q}`, p), nil
		},
	}
