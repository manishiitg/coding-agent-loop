...
| Reusable single-Crew setup | 65 generated Crew records with selected skill and pending chat checklist | Available locally; no customer setup is implied. |
| Multi-Crew proposal and typed handoffs | 29 locally installable multi-Crew Playbooks inside 47 total packages; 34 contract suites pass `python3 playbooks/scripts/validate_playbooks.py` | Authored and mechanically checked. Builder must still insert blocking validation in an actual Workflow. |
...
The other **18 of 47** installable Playbook packages have no `agent_slots`, `SETUP.json`, or executable handoff suite. They remain Workflow guides in the same catalog; they are not yet equivalent to the 29 chat-led multi-Crew proposals. The Playbook picker now labels the two kinds separately and states that Workflow guides have no predefined Crew team or tracked setup checklist; an installer test confirms this for Basic Browser Setup. This count excludes the example package under `playbooks/templates/`.
...
| Growth Analytics candidate (1) | Growth Experimentation and Follow-Through. | Approved experiment execution and outcome verification remain distinct from a pending proposal. Assess Crew ownership and typed evidence before launch promotion. |
...
These dispositions describe the current installable product, not a claim that the guides are operationally configured. The next authored candidates are the distinct experiment follow-through and post-incident jobs. Each needs a customer-like source case, rejected claim, owner and exact handoff before it is promoted.
