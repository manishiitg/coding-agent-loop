...
var referenceKinds = map[string]kindMeta{
	...
	"post-run-monitor":    {Group: "system", Description: "The post-run monitor playbook: read the run evidence + changelog + eval/metric files, form a Bug verdict (ran-right) and a Goal verdict (hits-success-criteria, gated on Bug being clean), update builder/improve.html (verdict pills, goal card, signal tiles, a run row, and a Monitor entry only if something's wrong); the verdict pills + headline in improve.html are the notification-gate signal. Read-only triage — diagnoses, never fixes. Load when running the post-run monitor pass.", Modes: []string{"workshop", "run"}},
