...
// WorkflowAnonymizationTemplate holds template variables for anonymization prompts
type WorkflowAnonymizationTemplate struct {
	WorkspacePath string
	VariablesJSON string
	VariableNames string
}

// WorkflowAnonymizationAgent scans learnings folder and replaces actual values with variable placeholders
type WorkflowAnonymizationAgent struct {
	*agents.BaseOrchestratorAgent
	baseOrchestrator *orchestrator.BaseOrchestrator // Reference to base orchestrator for RequestHumanFeedback
}

// NewWorkflowAnonymizationAgent creates a new anonymization agent
func NewWorkflowAnonymizationAgent(config *agents.OrchestratorAgentConfig, logger loggerv2.Logger, tracer observability.Tracer, eventBridge mcpagent.AgentEventListener, baseOrchestrator *orchestrator.BaseOrchestrator) *WorkflowAnonymizationAgent {
	...
	return &WorkflowAnonymizationAgent{
...
func (am *AnonymizationManager) createAnonymizationAgent(ctx context.Context, workspacePath string) (agents.OrchestratorAgent, error) {
	...
	createAgentFunc := func(cfg *agents.OrchestratorAgentConfig, logger loggerv2.Logger, tracer observability.Tracer, eventBridge mcpagent.AgentEventListener) agents.OrchestratorAgent {
		return NewWorkflowAnonymizationAgent(cfg, logger, tracer, eventBridge, am.BaseOrchestrator)
	}
...
func (agent *WorkflowAnonymizationAgent) Execute(ctx context.Context, templateVars map[string]string, conversationHistory []llmtypes.MessageContent) (string, []llmtypes.MessageContent, error) {
	...
	templateData := WorkflowAnonymizationTemplate{
		WorkspacePath: workspacePath,
		VariablesJSON: variablesJSON,
		VariableNames: variableNames,
	}
	...
	systemPrompt := agent.anonymizationSystemPromptProcessor()
...
func (agent *WorkflowAnonymizationAgent) anonymizationSystemPromptProcessor() string {
...
1. **Access Learnings Folder**: You have access to the learnings/ folder. You also have access to runs/ folder to discover and process step-specific folders. Variables are provided in template variables, not read from files.
...
func (agent *WorkflowAnonymizationAgent) anonymizationUserMessageProcessor(templateVars map[string]string) string {
...
**Remember**: 
- You have access to the learnings/ folder, and runs/ folder to discover step-specific folders
