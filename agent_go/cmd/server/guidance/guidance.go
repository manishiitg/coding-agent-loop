...
var allKinds = map[string]kindMeta{
	...
	"monitor":               {Group: "review", Description: "Post-run monitor: read the latest run + changelog + eval/metric files, record Bug + Goal verdicts (and any silent-failure/drift finding) into the workflow log. Read-only triage.", Modes: []string{"workshop", "run"}},
...
var referenceKinds = map[string]kindMeta{
	...
	"review-improve-log":  {Group: "system", Description: "The single workflow log spec: builder/improve.html format (newest-on-top timeline of run/monitor/decision/user-rule/note/open-finding entries, metrics as prose tiles, monthly archive), a copy-paste starter HTML skeleton, the prose close-out ritual, and the one-time migration off legacy .md / review.html / F-/I- ids / improve-decision blocks. Load before writing the log from any /review-* or /improve-* skill or the monitor.", Modes: []string{"workshop", "run"}},
	"post-run-monitor":    {Group: "system", Description: "The post-run monitor playbook: read the run evidence + changelog + eval/metric files, form a Bug verdict (ran-right) and a Goal verdict (hits-success-criteria, gated on Bug being clean), update builder/improve.html (verdict pills, goal card, signal tiles, a run row, and a Monitor entry only if something's wrong), and emit builder/monitor-verdict.json for the notification gate. Read-only triage — diagnoses, never fixes. Load when running the post-run monitor pass.", Modes: []string{"workshop", "run"}},
	...
	"org-pulse":           {Group: "system", Description: "The Org Pulse playbook (the Chief of Staff's daily heartbeat over the whole org): gather evidence in one sweep (each workflow's monitor-verdict.json + reports + knowledgebase + global learnings, recent conversation files, your own memory), roll the per-workflow Goal verdicts into an org endgame read, HARVEST what's worth keeping into your shared memory (curate/merge in your own words — never import/copy files), spot promotions (a repeated ad-hoc task → propose turning it into a workflow), and record everything (org health, harvested notes, and suggestion cards) in the single pulse/org-pulse.html log — no separate JSON — notifying only on a decision-worthy change. Read-only on workflow internals; judge/curate/suggest, never run or fix. Load when running the daily Org Pulse pass.", Modes: []string{"multi-agent"}},
	...
	"publish-strategy":      {Group: "system", Description: "Publish HTML artifacts (the Pulse log builder/improve.html or the org pulse/org-pulse.html, and the reporting dashboard) to a public URL on any static host — the share-twin of backup. Provider-agnostic + agentic (no per-provider code): three universal deploy paths (provider CLI like netlify/vercel/wrangler/gh-pages/surge/firebase; git-push-to-deploy; object-store/rclone/rsync sync), auth from a named secret. Includes the static-dashboard snapshot procedure (run the report's window.report.query SQL against db.sqlite, inline the results as JSON + a shim, deploy static — never ship the DB), a privacy/scope confirmation before exposing data, the configure->verify->auto flow, and the publish/status.json contract (with the public URL). Load when the user asks to publish/share/host a workflow's report or pulse, or to set up a publish destination.", Modes: []string{"multi-agent", "workshop", "run"}},
