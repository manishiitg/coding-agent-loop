...
Every plan-mod tool call is auto-written to `planning/changelog/changelog-*.json` (tool, `reason`, affected step ids, old/new values). Treat that changelog as the **ledger of changes whose blast radius may not be reconciled yet** — your work-list. When you reconcile a change (steps 1–3), **record its impact summary in `builder/improve.html`** so it's marked done.

Then **trim the changelog yourself.** Once you are confident a change's blast radius is fully reconciled *and* its impact summary is recorded, remove those reconciled entries — delete the fully-reconciled changelog file(s) with the shell on your own judgment. This is **agentic**: there is no automatic time-based prune to lean on, and you should not call one. The changelog should reflect changes **still needing reconciliation**, not an ever-growing history — keep it lean by removing what you've reconciled.

This proactive check is one end of a loop; the **`review-artifact-drift` audit** (the `review_artifact_sync` tool, which advances the **Artifact Sync Cursor** in `builder/improve.html`) is the agentic backstop that sweeps the changelog and catches anything the proactive pass missed. Pulse runs it as a separate report-only Artifact Review item, not as part of harden. Do the reconciliation when you make the change so the audit finds nothing to do — then the reconciled changelog tail is yours to trim.
