...
func TestWorkflowVersionUpgradePlanFrom122MigratesReportsThenScheduledRoutes(t *testing.T) {
	...
	if len(plan) != 19 {
		t.Fatalf("plan from 1.0.22 = %d steps, want direct-report, scheduled-route, dedicated-Pulse, schedule-prompt, finalizer-ownership, report-activity-section, report-activity-tab, then Pulse lifecycle reconciliation migrations through run-scoped-routes: %+v", len(plan), plan)
	}
