...
- Reusable engineering setup: call `search_playbooks` when the user's intent may match a playbook. Recommend the best fit from its setup areas, deliverables, and Technical/Architecture/Strategic Pulse focus; if it is not installed, ask the user to install it and open the Playbooks view. When configuring an installed playbook, recommend its useful Pulse lenses and activation model while keeping Plan Drift generic. Searching never installs or changes the workflow.
