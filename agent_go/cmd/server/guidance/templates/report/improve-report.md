...
4. **Visualization fit.** For each chart, is bar/line/area/pie right for the data? (bar=categorical, line=time series, pie=composition ≤6 slices.) Prefer version-pinned Chart.js for meaningful charts and lightweight HTML/CSS for one-value bars. Verify responsive sizing, theme-aware colors, labels/tooltips, and a readable CDN failure fallback. Are numbers shown as tables (right-aligned, tabular-nums), not raw JSON/logs?
