...
var builtinOrgPulseMessages = []string{
	`STEP 1/6 — BACKUP. You are running the daily Org Pulse, the Chief of Staff's heartbeat over the whole org. Call get_reference_doc(kind="org-pulse") and follow it; we go one step at a time — do ONLY this step, then stop. The user controls cadence by schedule, so do not skip the pass just because the org looks steady. Back up org-level artifacts per get_reference_doc(kind="backup-strategy") using pulse/backup.json and pulse/backup/status.json (same config/status split as workflow backup; set up the zero-config local-git default if backup is unconfigured; skip the push when the source hash is already backed up). Report the backup result, then stop.`,
	`STEP 2/6 — EVIDENCE + GOALS. In one efficient batched sweep, read pulse/goals.html (the goal scorecard), pulse/task.html (recent Chief of Staff task findings), and each workflow's builder/improve.html verdict pills + headline, latest reports, knowledgebase, and global learnings — follow get_reference_doc(kind="org-pulse") step 3 (curate evidence; trust the per-workflow verdicts; drill into raw runs only on a surprise). Then measure goals: for each goal compare current vs baseline/target, assign on-track / at-risk / off-track / unknown with a one-sentence reason, and diagnose the gap blocking each goal. Before writing, call get_reference_doc(kind="org-html"), then update pulse/goals.html as the durable current scorecard when concrete evidence changes status, latest evidence, confidence, freshness/last-reviewed, or history. Evaluate workflow alignment only when goals exist. Report the goal scorecard, each goal's gap, task findings used, and whether pulse/goals.html was updated, then stop.`,
	`STEP 3/6 — LLM + COST AUDIT (report-only). Main check: verify every workflow has a complete high/medium/low LLM tier setup, that each tier resolves to the expected provider/model, and that schedules or explicit overrides use the appropriate tier for the workflow's importance and current quality. In one batched sweep, inspect each workflow's workflow.json capabilities.llm_config / execution defaults / schedules, recent cost artifacts under costs/ and run folders, and any available report or Pulse evidence that names provider/model/tier. Treat mode="provider_profile" as complete via the coding-agent provider's current Builder, Maintenance, Pulse, and execution-tier defaults; require mode="explicit" to contain Builder, Maintenance, Pulse, and all three execution tiers. For each workflow, classify tier setup as complete, missing-tier, override-mismatch, over-tiered, under-tiered, or unknown; use costs/tokens only as supporting evidence for whether the tier choice is wasteful or underpowered. Summarize which workflows use provider profiles, explicit high/medium/low tiers, or per-step models, where tier evidence is missing, where cost evidence is missing, and any unusual mismatch between task importance and model tier. This is reporting only: do NOT change workflow.json, prompts, plans, model settings, schedules, or secrets; do NOT run optimizers or fixes. Report the LLM/cost scorecard and the evidence paths you used, then stop.`,
	`STEP 4/6 — GENERATE RECOMMENDATIONS + QUESTIONS. This is the proactive step: for each goal, propose grounded, prioritized recommendations to MOVE it — not just diagnose it. Follow get_reference_doc(kind="org-pulse") "Generate recommendations". First read existing org-level recommendation cards in pulse/goals.html and workflow-level Chief of Staff cards in each builder/improve.html (` + "`.cos-rec`" + ` / data-cos-rec-id / data-status). For any existing open rec, report whether it is proposed, accepted, queued_goal_advisor, in_progress, needs_evidence, blocked, done, or dismissed; update/follow up instead of duplicating. Every new rec is tied to a goal + evidence and ranked by impact/effort, and is PROPOSAL-ONLY — you recommend, the user/builder decides; never auto-apply a plan change. If a recommendation genuinely needs a user decision, clarification, or approval, call create_human_input_request(source="chief_of_staff", workspace_path="pulse" for an org-wide question or the affected Workflow/<name> for a workflow-specific question); do not ask only in HTML/email and do not block waiting for a response. Think beyond the obvious: a new automation for an unserved goal, a different approach for a capped goal, cross-automation synergies, and promotions. Write per-automation improvement recs into that automation's builder/improve.html as structured Chief of Staff recommendation cards with stable data-cos-rec-id, data-status="proposed", data-priority, data-suggested-action, evidence, gap, and expected impact. Write org-level recs to the Recommendations section of pulse/goals.html per get_reference_doc(kind="org-html"). Report recommendations written, questions created, existing recommendations followed up, stale open decisions, and where each lives, then stop.`,
	`STEP 5/6 — TASK FINDINGS + PROMOTIONS. Read pulse/task.html and extract recent key findings, open next actions, recurring asks, and cross-workflow patterns from normal Chief of Staff scheduled tasks. Then spot promotions: review task entries for a repeated ad-hoc task shape and PROPOSE turning it into a workflow (name it, generalize the procedure, cite instances) — propose only, do not create it. Follow get_reference_doc(kind="org-pulse") steps 7-8. Report task findings used and any promotion proposal, then stop.`,
	`STEP 6/6 — LOG + PUBLISH + DAILY DIGEST. Before writing, call get_reference_doc(kind="org-html") and use its rich widget-first Org Pulse skeleton. Prepend one dated entry to pulse/org-pulse.html (newest-first) covering the goal scorecard, workflow alignment delta, org-health one-liner, LLM/model tier + cost audit, recommendations summary, task findings/promotions, and suggestion cards using color-coded KPI tiles, priority lanes, evidence chips, and compact widgets instead of long prose. If org publish is configured in pulse/publish.json and already verified in pulse/publish/status.json, re-publish pulse/goals.html + pulse/org-pulse.html per get_reference_doc(kind="publish-strategy") and update pulse/publish/status.json (never do the first/verifying publish unattended). Notify the user with one daily Org Pulse digest whenever this pass reached step 6; decision-worthy changes affect severity and ordering, but a steady healthy day still gets a calm all-healthy digest. When Gmail/email is available, email is the default in-depth rendering: set email_subject, email_html (EMAIL-SAFE: inline styles only — Gmail strips <style>/<head>/class CSS, so do NOT paste pulse/org-pulse.html; build an inline-styled digest with workflow health, goal scorecard, workflow alignment, LLM/cost audit, recommendations, task findings/promotions, decisions needed, and links to Goals/Pulse when published), and plain email_body on the same notify_user call; set email_to only when the org notification preference asks to replace the default To recipient; set email_cc only when the org notification preference asks for CC recipients. Report the log entry, LLM/cost summary, publish result, and notification result, then stop.`,
}
	...
	return []WorkflowSchedule{
		{
			...
			Description:    "The Chief of Staff's daily heartbeat: review every workflow's outcome, task findings, and suggestions. Off by default; turn it on to opt in. Default once a day, editable.",
			CronExpression: "0 8 * * *",
			Timezone:       "UTC",
			Enabled:        false,
			Mode:           "multi-agent",
