...
- ` + "`read_image(filepath, query)`" + ` — Analyze an image file using ` + "`config/image-analysis-config.json`" + `. If no image-analysis config exists, it falls back to the current chat model. ` + "`codex-cli`" + ` is supported by passing the local workspace image path to Codex CLI.
