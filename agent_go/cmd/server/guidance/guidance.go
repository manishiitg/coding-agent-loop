...
var referenceKinds = map[string]kindMeta{
	...
	"gmail-connection-scopes": {Group: "system", Description: "Managing a Gmail/Google Workspace connection's permissions: checking current state with list_gmail_connections (stored request vs what Google actually granted, and the exact diagnosis when they've drifted — including the Google Cloud Console consent-screen-scope-registration gotcha), how to widen or narrow scope from chat with update_gmail_connection_grants, why a reconnect link is required afterward, and how a workflow's own service allowlist relates to the connection's. Load before changing, explaining, or troubleshooting a Gmail connection's Drive/Sheets/Docs/Slides/Calendar or read access.", Modes: []string{"multi-agent", "workshop"}, Tools: []string{"list_gmail_connections", "update_gmail_connection_grants"}},
