...
| Multi-Crew proposal and typed handoffs | 31 locally installable multi-Crew Playbooks inside 47 total packages; 36 contract suites pass `python3 playbooks/scripts/validate_playbooks.py` | Authored and mechanically checked. Builder must still insert blocking validation in an actual Workflow. |
...
The other **16 of 47** installable Playbook packages have no `agent_slots`, `SETUP.json`, or executable handoff suite. They remain Workflow guides in the same catalog; they are not yet equivalent to the 31 chat-led multi-Crew proposals. The Playbook picker now labels the two kinds separately and states that Workflow guides have no predefined Crew team or tracked setup checklist; an installer test confirms this for Basic Browser Setup. This count excludes the example package under `playbooks/templates/`.
