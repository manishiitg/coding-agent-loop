			...
			Parameters: llmtypes.NewParameters(map[string]interface{}{
				...
				"properties": map[string]interface{}{
					...
					"args": map[string]interface{}{
						...
						"description": "Arguments for the command. Normal/headless mode has no tab requirement: use standard agent-browser args such as ['https://google.com'] for open, ['-i'] for snapshot (interactive elements), ['@e1'] for click, ['@e2', 'search query'] for fill, ['Enter'] for press, ['page.png'] for screenshot, ['text', '@e1'] for get. In shared CDP mode only, choose/create a tab with command='tab' first; then call open with URL-only args. All other CDP page actions MUST include the tab inline before the command args: ['tab', '<tab-id-or-label>', ...] or ['--tab', '<tab-id-or-label>', ...]. CDP examples: ['tab', 't1', '-i'] for snapshot, ['tab', 't1', '@e1'] for click, ['tab', 't1', '@e2', 'search query'] for fill. If a CDP page action is called without a tab, the tool returns an error with the current tab list; then choose or create a tab and retry with the inline tab.",
					},
