...
var referenceKinds = map[string]kindMeta{
	...
	"schedules":               {Group: "system", Description: "Full schedule management reference: list/create/update/delete/trigger_schedule and get_schedule_runs; cron vs calendar schedules; hard max_run_duration_minutes limits; collision queues; after_schedule_ids chains and multi-schedule fan-in with status, delay, and deadline controls; mandatory per-schedule Pulse policy; backup coverage; and unattended route/message design. Load before creating, editing, or reasoning about any schedule.", Modes: []string{"workshop"}},
