...
var referenceKinds = map[string]kindMeta{
	...
	"skill-management":  {Group: "system", Description: "Install skills and attach them to workflows: find (list_skills/search_skills), install (install_skill/import_skill), attach workflow-wide (update_workflow_config add_skills) vs restrict per-step (update_step_config enabled_skills), the no-cascade attachment model, learnings/_global/SKILL.md as shared-know-how home, remove/uninstall, and troubleshooting. Load before installing a skill or wiring skills onto a workflow or step.", Modes: []string{"multi-agent", "workshop"}},
	...
	"reporting-policy":      {Group: "system", Description: "Live frontend report viewer policy: workshop owns report_plan.json widget authoring; the 'create a report / dashboard UI' dispatch (always update report_plan.json via report-plan tools, tabbed sections for routed workflows, never generate HTML/Python dashboards); 'No report yet' vs 'empty widgets' diagnosis; auto-refresh discipline; Run-mode boundary (no dashboard edits). Load when the user mentions reports, dashboards, widgets, themes, or layouts.", Modes: []string{"workshop", "run"}},
