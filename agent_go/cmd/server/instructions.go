...
- ` + "`planning/metrics.json`" + ` — quantified goal definitions. Each metric has id (kebab.dot), unit, direction (higher_better/lower_better), mode (target/slo with target/floor/ceiling), and a source (eval_step / telemetry / external / delayed_ground_truth). Optional ` + "`evaluable_at_lag`" + ` (e.g. ` + "`30d`" + `) declares a metric is delayed; the experiment loop waits for the lag to elapse. The top-level ` + "`active_mode`" + ` field is the runtime state for dual-mode workflows (e.g. explore/exploit cycles declared in improve.md) so steps can branch on it via the variable resolver. **Tool-only writes**: lives under ` + "`planning/`" + ` so the FolderGuard BlockedWritePaths makes shell writes impossible. The privileged ` + "`propose_metric`" + ` tool path goes through the workspace API and bypasses the block — call it to define/amend metrics OR to update ` + "`active_mode`" + ` (pass only ` + "`active_mode`" + ` to update just the mode; pass alongside a metric definition to do both in one write). NEVER attempt shell writes here — they will fail; even if they succeeded, they'd skip schema validation and version-archiving.
