...
3. **Optional - Check Execution Logs**: If you need more context about actual code execution, use execute_shell_command to read execution logs from '{{.ExecutionLogsPath}}' (if available). Execution logs contain:
...
4. **Consolidate**:
   ...
   - **Prune**: Use execute_shell_command with 'rm' to delete old/inefficient code files. **Keep ONLY the latest/best code file.**
...
5. **Persist**:
   ...
   - Use diff_patch_workspace_file to save the best code to '{{.CodePath}}/{{.StepTitle}}_code.py' (for Python) or use the appropriate extension for other languages (e.g., '.sh' for bash, '.go' for Go).
...
1. ⭐ **OPTIMAL**: [Pattern Name] [Runs: X | Success: Y%]
   ...
   - **Source**: 'code/{{.StepTitle}}_code.py' (or appropriate extension).
...
1. **CONSOLIDATE**:
   ...
   - Use execute_shell_command with 'cat' to read ALL existing '*_learning.md' files. Also check '{{.CodePath}}' for existing code files.
...
2. **EXTRACT THE BEST**:
   ...
   - Use diff_patch_workspace_file to save the best runnable code to '{{.CodePath}}/{{.StepTitle}}_code.py' (Python) or use the appropriate extension ('.sh' for bash, '.go' for Go, etc.).
   - **Dependency Rule**: For Python, use ONLY standard library or 'requests' (pre-installed). For bash/curl, use only standard system tools. Do NOT introduce new external dependencies.
   - **Error Handling**: Include robust error handling appropriate to the language (try/except for Python, 'set -euo pipefail' for bash, etc.) and HTTP response status checks.
   - **Task-Specific Failures**: Document what failed for *this specific task* (ignore general language errors).
3. **PERSIST & CLEAN**:
   - Use diff_patch_workspace_file to write ONE consolidated file to '{{.WritePath}}/{{.StepTitle}}_learning.md'.
   - Use execute_shell_command with 'rm' to delete all other '*_learning.md' and stale code files in these folders.
