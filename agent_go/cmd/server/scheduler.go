...
func postRunMonitorSteps() []postRunMonitorStep {
	return []postRunMonitorStep{
		...
		{"notify", "STEP 8 — NOTIFY a compact run summary once per Pulse run per the post-run-monitor doc's notification step. " +
			...
			"If this run needs user input, or the notify/email would ask the user a question, write or refresh a structured `Human input requested` entry in builder/improve.html before sending notify_user. The entry must use data-kind='input', data-question-id='<stable id>', data-status='open', and fields for Question, Why it matters, Options / expected answer, Default if no answer, Evidence, and Asked at; do not leave the question only in email/chat. Close or update prior matching question cards instead of duplicating them. " +
