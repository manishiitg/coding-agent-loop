...
2. a workshop Optimizer-mode **IMPROVE** schedule that wakes on a regular cadence (initially after every 1-2 runs, widening as the workflow stabilizes) and, **each fire, reads the log + cross-run Goal/strategy evidence and APPLIES what the evidence warrants** — it runs the `improve-workflow` decision model over the latest run/eval evidence and the per-run Bug/Goal findings, then:
   ...
   - **Evidence-chain freshness → APPLY.** Review the evidence chain across runs for stale or misleading **reports**, **learnings** (`learnings/_global/SKILL.md`), **KB** (`knowledgebase/notes/`), and **db** contracts. Use report-plan/HTML edits, `improve_learnings`, `improve_kb`, or `improve_db`. Reversible curation.
   - **Notify on decision-worthy changes.** Use `notify_user` once when the improve fire applies a structural replan, applies a user-facing report/eval/metric update, materially cleans KB/learnings/db, changes cadence/scope, or records a high-impact proposal/blocker. Stay silent on no-action/steady fires unless `soul.md ## Notifications` asks otherwise.
...
- valid group_names
- a clear name and description marking it the IMPROVE schedule (it APPLIES a strong-evidence structural replan and refreshes aging evidence-chain artifacts — reports, learnings, KB, and db; per-run hardening is Pulse's)
- a regular cadence (initial ≈ every 1-2 runs, widening as the workflow stabilizes)
- **no custom scheduled message is used**. Omit `messages` entirely for auto-improve; the scheduler owns the canonical improve prompt and ignores persisted optimizer messages. Do not store backup, publish, or notify turns in `workflow.json`.

The scheduler automatically injects the full runtime sequence, like Pulse does:
**STEP 1/5 pre-backup → STEP 2/5 canonical improve prompt → STEP 3/5 final backup → STEP 4/5 publish → STEP 5/5 notify**.
This keeps auto-improve as an on/off + cadence/group setting while the chat history still shows separate, diagnosable turns.

Do not set `messages` on the IMPROVE schedule. The scheduler always runs the current canonical five-turn Auto Improve sequence, even for older schedules that still have stale custom or operational messages saved. Each fire (mechanical, on cadence, whenever there is fresh run evidence to reason about) it:
...
- **Immediate Bug / reliability findings → Pulse/harden owns them:** per-run execution failures, broken step contracts, bad tool args, empty outputs, broken eval/report SQL, and other run-specific Bugs are Pulse's job because Pulse starts `harden_workflow` for them right after the run. This schedule does NOT call `harden_workflow`; it reads unresolved Bug findings as context and leaves immediate runtime repair to Pulse.
...
- **Evidence-chain freshness → APPLY bounded cleanup (reversible curation):** review the whole evidence chain across runs for staleness or misleading artifacts — report dashboards that no longer match live data, superseded or contradicted KB notes, stale selectors/tool patterns in learnings, duplicate/bloated guidance, and db contracts that drifted from plan/report usage. Apply the bounded action chosen by `improve-workflow`: report-plan/HTML edits with `validate_report_plan` + `preview_report_render`, `improve_learnings`, `improve_kb`, or `improve_db`. Cite the runs/notes/SQL/report preview that show the staleness; this curation is reversible (you backed up first). Do not disguise a runtime Bug as curation — if the producing step is wrong, leave it for Pulse/harden.
...
- does **not** call `notify_user` itself; the scheduler-injected final notify turn owns notification.
...
IMPROVE SCHEDULE PAYLOAD

For a normal `/auto-improve` schedule, omit `messages`. If an existing IMPROVE schedule already stores old custom or operational turns, clear `messages` when you touch the schedule for clarity; runtime ignores them either way and uses the canonical prompt.
...
After the opening check, the scheduler-owned improve prompt must call:
...
- this is a scheduled IMPROVE fire — APPLY when evidence is strong: back up first, then apply a structural replan from the cross-run Goal/strategy evidence when that evidence is STRONG (a clearly capped primary metric, repeated cross-run failure, or a materially better approach), citing the runs/evals/metrics that justify it; on thin/single-run evidence record a finding and widen cadence instead. Also APPLY evidence-chain freshness cleanup when the persisted artifacts are stale or misleading across runs: report accuracy/live-data fixes via report-plan/HTML edits, learning cleanup via `improve_learnings`, KB cleanup via `improve_kb`, and DB contract cleanup via `improve_db`. Honor a more cautious `oversight_mode` by keeping replan propose-only. Do NOT call `harden_workflow` — immediate per-run Bugs are Pulse's job; read unresolved Bug findings only as context unless `improve-workflow` classifies them as report/KB/learnings/DB evidence-chain cleanup rather than runtime repair
- do NOT call `notify_user` in runtime STEP 2/5 because the scheduler-injected STEP 5/5 is the dedicated notify turn. Instead, record a notification candidate in builder/improve.html when there was a decision-worthy improve change/proposal/blocker: action type, headline, evidence, expected impact, remaining risk, and whether default policy would notify.
...
- IMPROVE cadence: start ≈ every 1-2 runs. When recent fires increasingly surface no materially new strategy change and no stale evidence-chain artifacts to refresh (and metrics stay healthy), WIDEN the interval step by step (1-2 runs → 3-4 → weekly). When a material plan/config change lands (planning/changelog) or a regression appears, TIGHTEN back toward every 1-2 runs for 24-48h or until one or two post-change runs have been reviewed.
...
POST-FIRE BACKUP / PUBLISH / NOTIFY (scheduler-injected turns):
The IMPROVE schedule must not hide operational follow-up inside the improve turn and must not include extra schedule messages for it. The scheduler injects STEP 3 to back up the final state, STEP 4 to re-publish only an already-verified destination, and STEP 5 to perform the notification gate. This mirrors Pulse's separation and makes failures diagnosable: an improve can succeed while backup/publish/notify fails and leaves a clear chat turn plus status file.
...
- current notification policy from `soul.md ## Notifications`, if present
...
- that the improve schedule uses the scheduler-owned canonical prompt, and that the scheduler injects runtime STEP 1/5 pre-backup, STEP 3/5 final backup, STEP 4/5 publish, and STEP 5/5 notify
