...
func TestParseWhatsAppWorkflowCommandSessionEnd(t *testing.T) {
	for _, input := range []string{"@done", "@end", "@reset", "@new", "@newsession", "@quit", "@exit"} {
		cmd, arg, ok := parseWhatsAppWorkflowCommand(input)
		if !ok {
			t.Fatalf("expected %q to parse as workflow command", input)
		}
...
func TestWhatsAppCommandSuggestionsMentionDetailMode(t *testing.T) {
	...
	if !strings.Contains(list, "@full") || !strings.Contains(list, "@concise") || !strings.Contains(list, "@done") {
		t.Fatalf("workflow list should mention detail mode commands: %q", list)
	}

	unknown := unknownWhatsAppWorkflowCommandMessage("bad")
	if !strings.Contains(unknown, "@full") || !strings.Contains(unknown, "@concise") || !strings.Contains(unknown, "@done") {
