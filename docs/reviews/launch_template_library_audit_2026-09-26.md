...
| Reusable single-Crew setup | 64 generated Crew records with selected skill and pending chat checklist | Available locally; no customer setup is implied. |
| Multi-Crew proposal and typed handoffs | 28 locally installable multi-Crew Playbooks inside 47 total packages; 33 contract suites pass `python3 playbooks/scripts/validate_playbooks.py` | Authored and mechanically checked. Builder must still insert blocking validation in an actual Workflow. |
...
The other **19 of 47** installable Playbook packages have no `agent_slots`, `SETUP.json`, or executable handoff suite. They remain Workflow guides in the same catalog; they are not yet equivalent to the 28 chat-led multi-Crew proposals. The Playbook picker now labels the two kinds separately and states that Workflow guides have no predefined Crew team or tracked setup checklist; an installer test confirms this for Basic Browser Setup. This count excludes the example package under `playbooks/templates/`.
...
| Growth Analytics candidates (2) | Activation and Retention Intelligence; Growth Experimentation and Follow-Through. | Distinct remaining jobs: cohort retention review and approved experiment execution/outcome verification. Existing first-value, campaign and funnel routes do not prove either end to end. Assess their Crew ownership and typed evidence before launch promotion. |
...
These dispositions describe the current installable product, not a claim that the guides are operationally configured. The next authored candidates are the distinct retention, experiment follow-through and post-incident jobs. Each needs a customer-like source case, rejected claim, owner and exact handoff before it is promoted.
