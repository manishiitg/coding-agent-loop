Critical audit of the workflow design — the comprehensive review. Where /design-plan asks "what would a designer make better," this asks "what's wrong, weak, risky, stale, or unjustified, and which steps or artifacts need attention." Review the plan plus the dependent artifacts that make it executable: step config, skills, learnings, saved scripts, KB notes, db JSON files, report wiring, variables, and evaluation coverage.{{if eq .WorkshopMode "run"}} In Run mode, return findings in chat only; do not write files.{{else}} Findings go to builder/improve.html as open-finding entries (recommendations); nothing is applied here.{{end}}{{if .Focus}} Focus especially on: {{.Focus}}.{{end}}

Load `get_reference_doc(kind="assumption-audit")` and apply its plan/design lens. In addition to literal hardcoding, identify fixed architecture, channels, sources, thresholds, cadence, routes, and step boundaries that look mandatory only because an agent embedded them in several artifacts. Preserve explicit user constraints; report consequential unverified restrictions under Pulse's Assumptions challenged instead of treating repetition as proof.

Write findings into `builder/improve.html` as "Open finding" timeline entries. **Read `builder/improve.html` first** and carry forward unresolved findings rather than duplicating them. For the log format, one-time old Markdown migration, and how open findings are recorded and closed out, follow `get_reference_doc(kind="review-improve-log")` (and `get_reference_doc(kind="html-output")` for HTML style).
...
- **Non-converging branches (fall-through)**: a routing step jumps INTO the selected branch but does not stop the engine continuing in list order. So the **terminal step of each branch must rejoin** via `next_step_id` (point to the shared downstream step, or `"end"`). Flag CRITICAL when a branch's last step has no `next_step_id` and another step follows it in the list — the selected branch will run and then execution will fall through into the *next* (non-selected) branch. Multiple branches sharing one `next_step_id` (convergence to a common finish) is the correct shape. If every branch is just "run one sub-agent then continue the same way," recommend a `todo_task` orchestrator instead.
...
3. **Learning audit**:
   ...
   - Browser work should default to `agentic`. A user-requested scripted browser step under test is a note, not a violation; flag browser `scripted` that was not user-requested, and flag freezing it (`lock_code`) / relying on it without a defensible 10+ run scenario-coverage exception.
   - Flag `scripted` that was **not user-requested** (auto-promoted on the builder/optimizer's own initiative). A user-explicitly-requested scripted step is fine — they own that call and need it scripted to gather run evidence; do not flag it just for lacking 10 runs. Instead flag **premature `lock_code`**: freezing the script (disabling the repair loop) before 10+ scenario-covering successful runs with eval evidence at target, or while recent eval/run evidence is inconclusive / the behavior is still changing.
...
5. **Database audit**:
   ...
   - **Message-sequence item write access**: for every `message_sequence` step, check each conversational item whose `message` writes to `db/` or `knowledgebase/`. That item MUST declare matching `write_access` (`{"db": true}` and/or `{"knowledgebase": true}`) or `kind` (`db` / `knowledgebase`). Item writes are default-deny and folder-scoped — booleans only; a per-file `paths` list is invalid. Flag CRITICAL: name the item id and the store it tries to write. Also flag any legacy `type: "code"` item: it must be migrated by workflow contract v1.0.10 into a standalone scripted regular step.
