...
import type { MuseQuestionPrompt } from '../utils/cleanConversation'

export function MuseQuestionCard({ prompt, onAnswer }: {
  prompt: MuseQuestionPrompt
  onAnswer?: (promptId: string, answers: Array<{ id: string; selectedLabel: string }>) => Promise<void>
}) {
  const [selected, setSelected] = useState<Record<string, string>>({})
  const [submitting, setSubmitting] = useState(false)
  const [error, setError] = useState('')
  useEffect(() => { setSelected({}); setSubmitting(false); setError('') }, [prompt.promptId])
  const pending = prompt.state === 'pending'
  const ready = prompt.questions.every((question) => selected[question.id])
  const submit = async () => {
    if (!pending || !ready || !onAnswer || submitting) return
    setSubmitting(true)
    setError('')
    try {
      await onAnswer(prompt.promptId, prompt.questions.map((question) => ({ id: question.id, selectedLabel: selected[question.id] })))
  ...
  return <section className="w-full rounded-2xl border border-violet-300 bg-violet-50 p-4 text-slate-900 dark:border-violet-800 dark:bg-slate-900 dark:text-slate-100" data-testid="muse-question-card">
    <div className="mb-3 text-xs font-semibold uppercase tracking-wide text-violet-700 dark:text-violet-300">Muse needs your choice</div>
    {prompt.questions.map((question) => <fieldset key={question.id} className="mb-4 space-y-2" disabled={!pending || submitting || !onAnswer}>
      ...
      {question.options.map((option) => {
        const checked = pending ? selected[question.id] === option.label : prompt.answers.some((answer) => answer.id === question.id && answer.selectedLabel === option.label)
        return <label key={option.label} className={`flex cursor-pointer gap-2 rounded-lg border px-3 py-2 text-sm ${checked ? 'border-violet-500 bg-violet-100 dark:bg-violet-950' : 'border-slate-200 dark:border-slate-700'}`}>
          <input type="radio" name={`${prompt.promptId}:${question.id}`} value={option.label} checked={checked} onChange={() => setSelected((current) => ({ ...current, [question.id]: option.label }))} />
