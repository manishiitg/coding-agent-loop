...
A **Crew template** provides a reusable capability pack: a local skill, starter instructions, example requests, expected outputs, and a setup checklist. A new Crew may start from one template to seed its identity. An existing Crew can add more templates without changing its role or purpose. Each pack keeps its own setup progress. An **Automation** runs repeatable work against a goal and measures its results. A template may recommend Automations, but creating a Crew does not create or start one.
